...
interface FoodDetailApi {
    @Operation(
        summary = "음식 상세 조회",
        description = """
            한국어 메뉴명(menuName)으로 음식 상세를 조회한다. 요청 언어(lang)에 맞춰 음식명·재료명을 반환하며,
            지원하지 않거나 지정하지 않은 언어는 한국어(ko)로 폴백한다.
    ...
    @ApiResponses(
        value = [
            ApiResponse(responseCode = "200", description = "조회 성공 — 요청 언어 음식명·재료 목록 반환"),
            ApiResponse(
                responseCode = "400",
    ...
    @GetMapping("/detail")
    fun detail(
        @Parameter(description = "조회할 한국어 메뉴명(앞뒤 공백은 trim)", required = true, example = "된장찌개")
