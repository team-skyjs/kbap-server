...
- **인스턴스**: `cluster.tf` — api ASG(`api_instance_count`~`api_instance_max_count`) 를 capacity provider `kbap-<env>-ecs-api` 가 조정한다. `target_capacity = 50` 이라 인스턴스를 절반만 채워 그린 자리를 상시 보장한다. batch 풀은 고정.
