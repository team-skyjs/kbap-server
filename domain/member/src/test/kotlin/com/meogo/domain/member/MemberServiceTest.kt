        ...
        given("saveNew — 회원·신원 저장") {
            `when`("신규 회원을 저장하면") {
                then("member 한 행에 신원이 저장되고 findByIdentity 로 복원된다") {
                    val saved = service.saveNew(newMember(googleIdentity()))

                    saved.id.shouldNotBeNull()
                    val found = service.findByIdentity(SocialProvider.GOOGLE, "google-sub-1")
            ...
            `when`("빈 프로필로 가입하면") {
                then("맵기 선호 기본값 5 와 빈 기피성분이 복원된다") {
                    val saved = service.saveNew(newMember(googleIdentity(sub = "empty-profile-sub")))
        ...
        given("프로필 값이 채워진 회원 저장") {
            `when`("기피성분·맵기·국가·언어·닉네임을 담아 저장하면") {
                then("findById 재조회 시 profile JSON 값이 그대로 복원된다") {
                    ...
                    val saved = service.saveNew(toSave)
        ...
        given("동일 (provider, providerUid) 중복 저장") {
            `when`("같은 소셜 계정을 다시 saveNew 하면") {
                then("유니크 제약이 막아 DUPLICATE_SOCIAL_IDENTITY 예외를 던진다") {
                    service.saveNew(newMember(googleIdentity(sub = "dup-sub")))

                    val e = shouldThrow<MemberException> {
                        service.saveNew(newMember(googleIdentity(sub = "dup-sub", email = "other@gmail.com")))
                    }
        ...
        given("update — 프로필·온보딩 상태 갱신") {
            `when`("빈 프로필 회원의 프로필을 채우고 온보딩을 완료해 update 하면") {
                then("findById 재조회 시 갱신 값이 그대로 반환된다") {
                    val saved = service.saveNew(newMember(googleIdentity(sub = "update-sub")))
                    ...
                    service.update(saved.updateProfile(filled).completeOnboarding())
        ...
        given("소프트삭제된 회원") {
            `when`("findById·findByIdentity 로 조회하면") {
                then("탈퇴 회원은 반환되지 않는다") {
                    val saved = service.saveNew(newMember(googleIdentity(sub = "deleted-sub")))
                    softDeleteMember(saved.id!!)

                    service.findById(saved.id!!).shouldBeNull()
                    service.findByIdentity(SocialProvider.GOOGLE, "deleted-sub").shouldBeNull()
                }
            }
        }
        ...
        given("회원 탈퇴 — 소프트 삭제 + 신원 더미 치환") {
            `when`("활성 회원을 탈퇴 처리하면") {
                then("조회에서 제외되고 같은 소셜 계정 재가입이 유니크 충돌 없이 성공한다") {
                    val saved = service.saveNew(newMember(googleIdentity(sub = "withdraw-sub")))

                    service.withdraw(saved.id!!)

                    service.findById(saved.id!!).shouldBeNull()
                    service.findByIdentity(SocialProvider.GOOGLE, "withdraw-sub").shouldBeNull()

                    val resignup = service.saveNew(newMember(googleIdentity(sub = "withdraw-sub")))
            ...
            `when`("같은 소셜 계정으로 가입·탈퇴를 반복하면") {
                then("삭제 표식이 회원마다 유일해 탈퇴 행끼리도 충돌하지 않는다") {
                    val first = service.saveNew(newMember(googleIdentity(sub = "repeat-sub")))
                    service.withdraw(first.id!!)

                    val second = service.saveNew(newMember(googleIdentity(sub = "repeat-sub")))
                    service.withdraw(second.id!!)

                    val third = service.saveNew(newMember(googleIdentity(sub = "repeat-sub")))
            ...
            `when`("존재하지 않는 회원을 탈퇴 처리하면") {
                then("MEMBER_NOT_FOUND 예외를 던진다") {
                    val e = shouldThrow<MemberException> { service.withdraw(999_999L) }
        ...
        given("회원 정지 상태 — 소프트 삭제와 별개 축") {
            `when`("신규 회원을 저장하면") {
                then("member_status 는 기본 ACTIVE 다") {
                    val saved = service.saveNew(newMember(googleIdentity(sub = "status-default-sub")))
            ...
            `when`("정지된 회원을 서비스 조회 경로로 조회하면") {
                then("findById·findByIdentity 에서 제외된다") {
                    val saved = service.saveNew(newMember(googleIdentity(sub = "suspended-sub")))
                    suspendMember(saved.id!!)

                    service.findById(saved.id!!).shouldBeNull()
                    service.findByIdentity(SocialProvider.GOOGLE, "suspended-sub").shouldBeNull()
                }
            }

            `when`("정지된 회원을 관리자 조회 경로로 조회하면") {
                then("정지 회원이 그대로 보이고 소프트 삭제 상태는 ACTIVE 로 남는다") {
                    val saved = service.saveNew(newMember(googleIdentity(sub = "admin-view-sub")))
                    suspendMember(saved.id!!)
        ...
        given("스캔 횟수 — 랭킹 카운트") {
            `when`("가입 직후 회원을 저장하면") {
                then("스캔·리뷰·고유 음식 카운트가 모두 0으로 초기화된다") {
                    val saved = service.saveNew(Member.signUp(googleIdentity()))
            ...
            `when`("리뷰 카운트가 쌓인 회원을 조회하면") {
                then("컬럼 값이 도메인으로 복원된다") {
                    ...
                    val ranking = service.findById(saved.id!!)!!.ranking
            ...
            `when`("스캔 횟수를 올리면") {
                then("DB 에서 직접 1씩 증가하고 다시 조회해도 유지된다") {
                    ...
                    service.increaseScanCount(saved.id!!)
                    service.increaseScanCount(saved.id!!)

                    service.findById(saved.id!!)!!.ranking.scanCount shouldBe 2
            ...
            `when`("카운트업 이전에 읽어 둔 회원으로 프로필을 갱신하면") {
                then("그 사이 올라간 스캔 횟수를 덮어쓰지 않는다") {
                    val saved = service.saveNew(Member.signUp(googleIdentity()))
                    val stale = service.findById(saved.id!!)!!

                    service.increaseScanCount(saved.id!!)
                    service.update(stale.updateProfile(MemberProfile.empty()))

                    service.findById(saved.id!!)!!.ranking.scanCount shouldBe 1
                }
            }

            `when`("존재하지 않는 회원의 스캔 횟수를 올리면") {
                then("MEMBER_NOT_FOUND 로 거절된다") {
                    val e = shouldThrow<MemberException> { service.increaseScanCount(999999L) }
