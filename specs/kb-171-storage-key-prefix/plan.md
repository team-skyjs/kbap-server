...
├── application.yml                   # [수정] kbap.storage.key-prefix: ${STORAGE_KEY_PREFIX:} 선언
├── application-dev.yml               # [수정] key-prefix: ${STORAGE_KEY_PREFIX:} 선언(가시성)
└── application-staging.yml           # [수정] key-prefix: ${STORAGE_KEY_PREFIX:} 선언(가시성)
```
...
4. **yml 선언 전략** — base `application.yml` 에 `${STORAGE_KEY_PREFIX:}` 를 한 번 선언(전 환경 env 반응 + 기본 빈 값), dev·staging 프로필에도 동일 선언(DoD 명시 — 환경별 설정 가시성, KB-169 의 프로필별 명시 선언 관례). prod·local 은 미선언 = 빈 값.
