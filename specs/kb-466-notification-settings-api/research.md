...
- **Rationale**: 활동/소식 토글 하나가 도움됨·리마인더를 함께 제어(FR-004), 식사 시간 알림 토글 하나가 점심·저녁을 함께 제어(FR-005). 소식 켜기는 하위 토글을 전부 켠다(`updateMealTime(true)`, FR-008 — 2026-09-07 결정). 응답의 `mealTime` 은 `저장값 AND 소식 켜짐` 으로 계산해 소식이 꺼진 회원에겐 항상 false 로 보인다(FR-002).
