...
- [x] T020 [US1] `api/src/main/kotlin/com/kbap/api/notification/NotificationSettingController.kt` 신규(T017·T018·T019 후) — `@RestController @RequestMapping(ApiPaths.API + "/notifications")`, `@GetMapping("/settings", version = "1.1+") fun getSettings(@AuthMemberId memberId: Long): ResponseEntity<BaseResponse<NotificationSettingsResponse>>`.
...
- [x] T026 [US2] `NotificationSettingController.kt` 에 `@PatchMapping("/settings", version = "1.1+") fun updateSettings(@AuthMemberId memberId: Long, @RequestHeader(ApiHeaders.INSTALLATION_ID, required = false) installationId: String?, @Valid @RequestBody request: NotificationSettingsUpdateRequest)` 추가, `NotificationSettingApi.kt` 에 오퍼레이션 문서(400 COMMON-002·NOTIFICATION-001 포함).
