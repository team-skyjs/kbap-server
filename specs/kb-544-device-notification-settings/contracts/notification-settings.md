# Contract: GET·PATCH /api/notifications/settings (X-API-Version 2.1+)

경로·응답 형태는 구 계약(무버전 매핑, 1.0~2.0)과 같다. 달라지는 것은 세 가지 — `X-Installation-Id` 헤더가 **필수**, 값이 **이 기기의 설정**, 그리고 소식 그룹에서 **기기 수신(`enabled`)과 회원 동의(`consent`)가 별개 항목**이다(2026-09-11 FE 검토 반영).

## 변경 요약

| 항목 | 구 계약(≤ 2.0) | 새 계약(2.1+) |
|------|----------------|---------------|
...
| 요청 본문 | `NotificationSettingsUpdateRequest` | `DeviceNotificationSettingsUpdateRequest`(위 필드) |
| 응답 본문 | `NotificationSettingsResponse` | 동일 |
...
| US1-4 | `X-API-Version: 1.1` GET/PATCH | 구 계약 동작(NULL 행) — 기존 테스트 무변경 통과 |
...
## Swagger 서술(`NotificationApi` 신규 메서드 2개)

- `getDeviceSettings` — "**2.1 이상** — `X-Installation-Id` 필수. 이 기기의 설정을 돌려준다. 설정이 없는 기기는 전부 false 이고 기록을 만들지 않는다. `news.enabled` 는 이 기기 소식 토글 저장값이고, 동의 상태는 `privacyConsent`·`receiveConsent`(회원 단위, 종류별 열린 최신)로 읽는다. 발송은 기기 토글 AND 회원 동의 유효(버전 ≥ 요구치)를 검사한다."
- `updateDeviceSettings` — 위 변경 요약의 `consent`(동의 켜기/끄기, 회원 단위)·`enabled`(이 기기 수신)·mealTime 선행 조건·처리 순서. "2.0 이하 헤더는 종전 회원 단위 계약(소식 토글·동의 결합)으로 처리된다."
- 구 메서드 서술에 "2.1 이상은 기기 단위 계약으로 대체됐다" 한 줄 추가.
