...
- **구현 확인(2026-09-07)**: 헤더 파라미터에 `@NotBlank @Size` 를 달면 Hibernate Validator 가 `ConstraintDeclarationException`(HV000151 — 인터페이스 메서드에 없는 파라미터 제약을 구현이 추가) 으로 500 을 낸다. 규약상 인터페이스에는 swagger 애너테이션만 두므로, 공백·36자 초과는 컨트롤러의 `validInstallationId` 가드(`BusinessException(INVALID_REQUEST)` → 400)로 막는다(`ScanV2Controller.requestedCurrency` 선례).
...
- **Decision**: `PUT /api/notifications/tokens` 는 `@PutMapping("/tokens", version = "1.1+")` 로만 매핑한다(무버전 매핑 없음 → `1.0` 요청은 **404**). `AuthController` 의 기존 `login`·`logout`·`withdraw` **메서드 하나를 그대로 공유**하고, `@RequestHeader(X-API-Version)` 로 버전을 받아 **핸들러 안에서 분기**한다(2026-09-07 사용자 결정 — 버전별 매핑 복제 대신 헤더 분기). `ApiVersions.isAtLeast(apiVersion, "1.1")` 이면 `installationId`/`releaseDevices = true` 를 `AuthService` 에 넘기고, 아니면 기본 인자(null/false) 경로 = 종전 1.0 동작. 비교는 Spring 의 `SemanticApiVersionParser` 로 하며(`"1.10" > "1.9"` 같은 문자열 비교 함정 회피), 헤더는 `/api/**` 에서 필수·검증 완료 상태로 핸들러에 도달한다. springdoc 은 이미 있는 `X-API-Version` 파라미터를 중복 추가하지 않으므로(`apiVersionHeaderCustomizer` 의 `none { it.name == ... }` 가드) `AuthApi` 에 `@Parameter` 로 1.0/1.1 차이를 서술한다.
...
- **테스트 영향**: MockMvc 기본 헤더가 `1.0`(`MockMvcDefaultVersionConfig`)이라 새 테스트는 `X-API-Version: 1.1` 을 명시한다. 기존 `AuthControllerTest`(1.0)는 무수정으로 Green 이어야 하며, 이것이 "1.0 무영향" 의 회귀 증거다. **구현 확인(2026-09-07)**: 1.0 요청은 버전 조건에 맞는 핸들러가 없어 `NoResourceFoundException` → **404 COMMON-002** 다(400 이 아님). 같은 경로에 다른 버전 매핑이 하나도 없으면 Spring 은 버전 불일치가 아니라 매핑 부재로 취급한다. "1.0 에는 존재하지 않는 API" 라는 의미와 일치하므로 그대로 둔다 — 400 을 만들려면 무버전 더미 매핑이 필요해 과하다.
- **Alternatives considered**: 1.0 핸들러도 헤더가 있으면 기기를 연결 — "1.0 무영향" 요구 위반. 같은 경로에 `version = "1.1+"` 매핑 3개를 추가(`MemberController` 온보딩 선례, 초안 구현) — 응답·본문이 같은 엔드포인트를 메서드 3개로 복제하게 되어 사용자 지시로 헤더 분기로 교체. `AuthService` 메서드를 1.0/1.1 두 벌로 분리 — 로직 중복.

## R12. Codex 독립 리뷰 결과와 처리 (2026-09-07, PR #247)

- **수정 1 — 탈퇴 회원의 미만료 access 토큰으로 기기 재연결**: `@AuthMemberIdOrNull` 은 회원 활성 여부를 보지 않아, 1.1 탈퇴로 연결을 끊은 직후 30분(access TTL) 안에 같은 토큰으로 PUT 하면 기기가 탈퇴 회원에 다시 연결됐다. `registerToken` 이 회원 요청일 때 `memberService.getMember`(활성 회원만 노출하는 계약)를 먼저 호출해 `MEMBER-003` 400 으로 거절한다 — 다른 인증 엔드포인트와 같은 동작. 테스트 추가.
- **수정 2 — 동의 버전 상한**: DTO 가 `@Positive` 만 검사해 `65536` 이 저장 단계(`SMALLINT UNSIGNED`)에서 터지며 기기 등록까지 롤백됐다. `@Max(65535)` 추가, 경계값 테스트.
- **수용 1 — 로그아웃이 회원 검증 없이 헤더만으로 unlink**: spec 가정(로그아웃은 refresh 토큰만 받는 계약 유지, 기기 해제는 헤더만으로)대로다. 위협은 타인의 설치 UUID(기기 밖으로 나가지 않는 128비트 난수)를 알아야 성립하고, 효과는 회원 알림이 잠시 끊기는 것뿐이며 다음 앱 실행의 회원 PUT 이 재연결한다. 반대로 refresh 가 만료된 로그아웃에서 해제를 생략하면 로그아웃한 기기가 회원 알림을 계속 받는 spec 의 핵심 결함이 되살아난다. 현행 유지.
- **수용 2 — 게스트 PUT 이 회원 기기의 토큰을 교체**: 같은 위협 모델(설치 UUID 필요). 게스트 PUT 이 회원 연결을 유지하며 토큰을 갱신하는 것은 회원 기기의 정상 재실행 경로라 spec 이 명시한 동작. 기기 자격 증명(서명)은 에픽 범위 밖 — 필요해지면 별도 태스크.
