...
- `items` 1~50개 필수. `menuName` 필수(≤100자), `1 ≤ quantity ≤ 999`, `0 ≤ price ≤ 10,000,000` 옵셔널, `foodId` **필수**(미등록 음식도 스캔 시점에 등록되므로 스캔 응답의 foodId 를 그대로 보낸다 — **음식 상태(준비중 포함)와 무관하게 존재하는 음식이면 통과**, 존재하지 않는 id 만 400 `ORDER-001`. KB-370 에서 READY 제약 제거).
