...
4. **스키마** — `food_view_log(id, food_id NOT NULL, member_id NULL, status, created_at, updated_at, reserved_1~3 VARCHAR(255) NULL)`. 인덱스 `idx_food_view_log_created_food (created_at, food_id)` 하나 — "기간 창 → 음식별 GROUP BY"(FR-008) 가 창 안의 행만 커버링 스캔. 회원별·음식별 시간 범위 조회는 요구 없음 → 인덱스 없음. (Codex 리뷰로 선두 컬럼을 created_at 으로 교체.) `reserved_1~3` 은 사용자 결정(2026-09-23)에 따른 예비 컬럼 — 유입 경로 등을 나중에 담을 자리이며 엔티티에 매핑하지 않는다(research R7).
