...
- [X] T002 [P] `common/src/main/kotlin/com/kbap/common/domain/community/PostingJpaRepository.kt` 에 신규 쿼리 2개 추가: `findPage(cursor: Long?, pageable: Pageable): List<Posting>` (`@Query` — cursor null 이면 전체, 아니면 `id < :cursor`, id DESC) · `findIdsFrom(cursor: Long, pageable: Pageable): List<Long>` (`@Query` id 프로젝션 — `id >= :cursor` ASC, 게이트용 LIMIT 판정)
...
- [X] T011 [US2] `CommunityService.getPostingPage` 에 게이트 삽입 — `viewerMemberId == null && cursor != null` 이면 `findIdsFrom(cursor, PageRequest.of(0, PAGE_SIZE + 1)).size > PAGE_SIZE` 일 때 `BusinessException(ErrorCode.COMMUNITY_LOGIN_REQUIRED)` (research.md R2 — LIMIT 21 프로젝션으로 최악 스캔 고정). T010 실행해 **Green 확인**
...
Phase 3 US1 (T005 Red → T006·T007 병렬 → T008 → T009 Green)   🎯 MVP
   ↓
Phase 4 US2 (T010 Red → T011 Green)   ← US1 의 getPostingPage 에 게이트 삽입
