...
스캔 API(`POST /api/v1/scans`)의 응답 언어를 회원 프로필 저장값(`MemberProfile.appLanguage`, 미설정 시 KO)에서 **`lang` 쿼리 파라미터**(필수, 미지원 코드 → en 폴백 — 헌법 원칙 V/KB-201 정책)로 전환하고, 프로필의 마지막 언어 사용처가 사라지므로 **`appLanguage` 를 회원 프로필에서 전면 제거**한다(온보딩 입력·프로필 수정 입력·내 프로필 조회 응답·도메인 모델·JSON 직렬화 모두). 기존 회원 profile JSON 에 남은 `appLanguage` 키는 마이그레이션 없이 무시한다 — 단, Hibernate JSON 역직렬화가 unknown key 에 깨지지 않도록 `MemberProfileJson` 에 관용 역직렬화를 명시한다(research.md R3).
...
**Storage**: MySQL — `member.profile` JSON 컬럼(스키마 변경 없음, Flyway 마이그레이션 불필요)
...
├── scan/
│   ├── ScanController.kt        # 수정 — @ModelAttribute ScanLangRequest 추가, LanguageCode 확정 후 서비스 호출
