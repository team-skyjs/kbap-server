...
- `payload.items[].name`: 매칭된 메뉴는 **요청 `lang` 의 번역**으로 내려간다(번역 부재 시 `ko` 폴백). 회원 프로필 설정은 더 이상 관여하지 않는다.
- 매칭되지 않은 메뉴의 `name`(인식 원문)·그 외 필드는 기존과 동일.
...
### POST /api/v1/members/onboarding (예시 경로 — 기존 온보딩 엔드포인트)
...
- `ScanApi`: `lang` 쿼리 파라미터 문서화(HomeRequest 와 동일 서술 — 지원 코드 목록·en 폴백).
