...
- **Decision**: `MemberProfileJson` 에서 `appLanguage` 필드를 제거하고 클래스에 `@JsonIgnoreProperties(ignoreUnknown = true)` 를 명시한다. 그리고 기존 row 의 키를 Flyway 마이그레이션(`JSON_REMOVE`)으로 제거한다. 두 조치를 함께 두는 이유는 롤링 배포 — 마이그레이션이 끝난 뒤에도 구 인스턴스가 `appLanguage` 를 포함해 다시 쓸 수 있어, 신 인스턴스가 그 row 를 읽어야 한다. 기존 row 의 `{"appLanguage":"ko",...}` JSON 을 읽는 회귀 테스트를 둔다.
...
- **Alternatives considered**: `appLanguage` 필드를 남기고 무시 — 죽은 필드가 계약에 남아 제거 목적 미달, 기각. 관용 역직렬화만 두고 마이그레이션 생략 — 동작은 안전하지만 폐기 키가 데이터에 영구히 남아 기각(KB-229 리뷰 반영).
