...
| II. Bounded Contexts | ✅ | 원장은 `:domain:scan` 소유(유일 생산자=스캔). 타 도메인 참조 없음(FK 없음, 독립 원장). 공유 vocabulary(이벤트)는 `:core` |
| III. Layered Dependency Direction | ✅ | `:infra:llm` → `:core`(이벤트 클래스) 발행, `:app:api` → `:domain:scan`(서비스) 소비. 역방향 의존 없음. 이벤트는 스프링 컨텍스트 경유라 infra→도메인 컴파일 의존 불발생 |
...
domain/scan/src/test/.../LlmCallCostServiceTest.kt                  # [신규] 영속·정밀도
app/api/src/test/.../scan/LlmCallCostEventListenerTest.kt           # [신규] 비동기 소비(eventually)·실패 격리
```

**Structure Decision**: 발행(:infra:llm)—이벤트(:core)—원장(:domain:scan)—소비(:app:api) 4모듈 터치, 신규 모듈 없음. 근거는 research.md R1~R4.
...
| `LlmCallCostServiceTest` | :domain:scan | record→행 저장(Testcontainers), DECIMAL 정밀도 왕복 보존 |
