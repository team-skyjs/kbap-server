...
- **Decision**: 집계 포트를 만들지 않는다. 정책의 카운트 3종을 모두 `member` 컬럼으로 두되, 리뷰 카운트 2종은 리뷰 기능이 붙을 때까지 0에 머문다.
...
- **Alternatives considered**:
  - Jira DoD 에 적힌 "리뷰 집계 포트 정의 + 0 반환 연결" — 지금 시점에 인터페이스 1개 + 구현 1개가 늘지만 얻는 게 없다. 카운트 컬럼만 미리 잡아 두면 리뷰 기능은 카운트업 호출만 추가하면 된다(마이그레이션·도메인·응답 계약 무변경).
...
- **카운트업은 DB 원자적 증가**(`update member set scan_count = scan_count + 1 where id = ?`). 회원을 읽어 +1 해서 저장하면 같은 회원의 동시 스캔에서 두 트랜잭션이 같은 값을 읽고 같은 값을 써 스캔이 유실된다(lost update). 락·낙관적 버전은 재시도 처리를 부르므로, 증가문 하나가 가장 짧고 정확하다. 같은 이유로 프로필 저장 경로는 카운트 컬럼을 쓰지 않는다(오래된 스냅샷이 카운트를 되돌리지 못하게).
- **Alternatives considered**:
  - 별도 카운터 테이블 + `INSERT ... ON DUPLICATE KEY UPDATE` — 동시성은 같은 수준으로 해결되지만, 애그리거트가 아닌 것에 리포지토리를 두게 되고 가입 초기화·탈퇴 정리를 따로 챙겨야 한다. 회원 컬럼 + 원자적 증가면 둘 다 공짜다.
  - 낙관적 락(`@Version`) 또는 비관적 락 — 증가 하나에 재시도/대기 로직이 붙는다. 증가문은 애초에 경합을 만들지 않는다.
