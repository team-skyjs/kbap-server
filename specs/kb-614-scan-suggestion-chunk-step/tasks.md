...
- [x] T001 Create port `data class PushReceipt(val ok: Boolean, val errorCode: String? = null, val message: String? = null)` + `fun interface PushReceiptFetcher { fun fetch(ticketIds: List<String>): Map<String, PushReceipt> }` in `common/src/main/kotlin/com/kbap/common/port/push/PushReceiptFetcher.kt` (Spring·JPA 의존 금지 — ArchUnit port 규칙)
- [x] T002 Create `FakePushReceiptFetcher`(`PushReceiptFetcher` 구현 — `receipts: MutableMap<String, PushReceipt>` 에 든 것만 돌려줌, `failWith: Throwable?` 가 있으면 던짐, 받은 id 기록 `requested`, `reset()`) + `@TestConfiguration FakePushReceiptFetcherConfig`(`@Bean @Primary`) in `batch/src/test/kotlin/com/kbap/batch/notification/FakePushReceiptFetcherConfig.kt` — 같은 폴더 `FakePushSenderConfig.kt` 와 같은 모양 (depends on T001)
- [x] T003 Add `FakePushReceiptFetcherConfig::class` to the `@Import` of `batch/src/test/kotlin/com/kbap/batch/BatchIntegrationTest.kt` (depends on T002)
