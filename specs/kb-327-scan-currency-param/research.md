...
## R2. 통화 결정 — 필수 파라미터 단일 출처, 프로필 fallback 없음 (2026-08-12 개정)

- **Decision**: `currency` 를 **필수** 파라미터로 두고 응답 통화는 파라미터만으로 결정한다. 스캔 경로는 `member.profile.currency` 를 읽지 않는다. 누락은 400(`COMMON-002`).
- **Rationale**: fallback 경로가 남으면 회원(프로필 있음)/비회원(프로필 없음)의 미전달 동작이 갈라져 계약이 둘이 된다. `lang` 필수 정책(헌법 V — 값을 안 보낸 실수는 시끄럽게 실패)과 동일 근거. 2.0 스캔 통화 정보(KB-323)는 아직 미출시라 릴리스 전 계약 확정으로 하위 호환 부담이 없다.
- **Alternatives considered**: 선택 파라미터 + 프로필 fallback(초안 결정, 첫 구현까지 진행) — 회원/비회원 동작 분기·조용한 출처 이원화로 사용자 지시에 따라 폐기. 프로필 우선·파라미터 보조 — 요구사항(프로필 설정대로 반환되지 않도록)과 정반대라 기각.
...
- **Decision**: `ScanV2Controller` 가 raw 문자열을 `CurrencyCode.from(raw) ?: throw BusinessException(INVALID_CURRENCY_CODE)` 로 확정하고, `ScanService` 시그니처는 확정 타입 `requestedCurrency: CurrencyCode` 로 받는다.
