...
2.0 스캔 요청에 **필수** 쿼리 파라미터 `currency`(ISO 4217 코드)를 추가하고, 응답의 통화 환산 정보(`ScanV2Response.currency`)를 **파라미터 단일 출처**로 결정한다 — 회원 프로필 통화는 스캔 경로에서 더 이상 읽지 않는다(fallback 없음, 2026-08-12 개정 — research R2). 누락은 400(`COMMON-002`), 지원하지 않는 값은 기존 `ErrorCode.INVALID_CURRENCY_CODE`(`MEMBER-010`, 400)로 스캔 실행 전에 실패시킨다.

설계 핵심(연구 결정 [research.md](research.md)): 헌법 원칙 V에 따라 **검증은 요청 경계가 소유한다** — `ScanV2Controller` 가 raw `currency` 문자열을 `CurrencyCode` 로 확정(잘못된 값이면 throw)해 넘기고, `ScanService` 는 확정 타입만 받는다. 통화 결정이 회원 프로필 존재에 의존하지 않아 비회원 스캔(후속 태스크)의 선행 조건이 된다. 1.0 스캔·응답 형태·환율 스냅샷(`CurrencyCode.krwPerUnit`)은 불변. DB·마이그레이션·인프라 변경 없음.
...
**Constraints**: `currency` 필수 — 프로필 fallback 경로 금지(회원/비회원 계약 단일화), 1.0 불변, 잘못된 값은 조용한 대체 금지(MEMBER-010 실패), 누락·잘못된 값 검증은 비전(LLM) 호출 전 수행. 2.0 은 미출시 계약이라 버전 번호 유지
...
| 원칙 | 판정 | 근거 |
|------|------|------|
| I. Test-First (NON-NEGOTIABLE) | PASS | 실패 테스트(파라미터 기준 환산·잘못된 값 400·누락 400) 선작성 → Red 확인 → 최소 구현. tasks 에서 test-writer → implementer 순서 고정 |
...
| V. Domain Content Language Policy | PASS | **검증 소유 조항 적용**: `currency` 필수 여부·유효성 판정은 요청 경계(`ScanV2Controller`)가 소유하고 `ScanService` 는 확정 타입(`CurrencyCode`)을 받는다 — `lang` 필수 정책과 동일 패턴(누락은 시끄럽게 실패). 통화 코드는 ISO 4217 식별자로 번역 콘텐츠 정책과 무관. lang 과 달리 미지원 값을 폴백하지 않는 근거는 research R3 |
...
├── ScanV2Controller.kt   # [수정] @RequestParam currency: String(필수) 추가,
...
└── ScanControllerTest.kt # [수정] 파라미터 기준 환산·잘못된 값 400·누락 400 시나리오 추가
