...
| `ScanResult` | `api.scan` | 스캔 유스케이스 결과 — 통화를 모른다 | `currency` 필드 제거 (KB-323 이전 형태) |
| `ScanV2Response.CurrencyResponse` | `api.scan` | 응답 DTO `{ code, krwPerUnit }` — 컨트롤러가 `from(result, currency)` 로 조립 | 필드 non-null 화·`@Schema` 갱신 |
...
[요청 경계 — ScanV2Controller, 스캔 실행 전]
파라미터 누락          → 400 COMMON-002 (Spring 필수 파라미터 검증)
CurrencyCode.from(raw) → 성공: requestedCurrency = 그 값
                       → 실패: BusinessException(INVALID_CURRENCY_CODE)

[응답 조립 — ScanV2Controller]
ScanV2Response.from(result, requestedCurrency) — 서비스·결과 타입은 통화를 모른다
```
