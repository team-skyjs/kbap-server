...
| `MemberProfile.currency` | `common.domain.member.model` | 회원 프로필 통화(`CurrencyCode?`, 미설정 가능) — fallback 출처 | 없음 |
...
```text
입력: currency 쿼리 파라미터(raw String?), 회원 프로필 통화(CurrencyCode?)

[요청 경계 — ScanV2Controller]
raw == null            → requestedCurrency = null
CurrencyCode.from(raw) → 성공: requestedCurrency = 그 값
                       → 실패: BusinessException(INVALID_CURRENCY_CODE) — 스캔 미실행

[유스케이스 — ScanService]
확정 통화 = requestedCurrency ?: member.profile.currency   (null 이면 응답 currency = null)
```
