...
```json
"review": { "averageRating": 0.0, "reviewCount": 0, "sameCountryAverageRating": 0.0, "blur": false }
```

### 회원 조회 — 전체 리뷰는 있으나 같은 국적 리뷰 없음(또는 국적 미보유 회원)

```json
"review": { "averageRating": 3.7, "reviewCount": 3, "sameCountryAverageRating": 0.0, "blur": false }
```

### 비회원 조회 — 리뷰 유무 무관 (서버측 가림)

```json
"review": { "averageRating": 0.0, "reviewCount": 0, "sameCountryAverageRating": 0.0, "blur": true }
```
...
3. `blur=true` 응답의 수치는 항상 기본값 — 클라이언트는 `blur` 로 "가려짐" 상태를, `blur=false && reviewCount==0` 으로 "리뷰 없음" 상태를 구분해 렌더링한다(FR-002).
4. 클라이언트는 필드 존재 여부가 아니라 값으로 분기한다 — `sameCountryAverageRating == 0.0` 은 "표시할 같은 국적 평점 없음"으로 취급(0.0 실평균 리뷰는 별점 1~5 제약상 존재 불가).
