...
└── domain/notification/
    ...
    ├── PushRequest.kt                                 # PushRequest.ttlSeconds?, PushEnvelope.channelId·ttlSeconds
...
│   │   ├── ScanSuggestionSendWindow.kt                # 08:00 ≤ t < 21:00 KST 순수 판정 + "오늘 KST 시작" 계산
│   │   ├── ScanSuggestionTargetTasklet.kt             # 가드 → 후보 조회 → 오늘 받은 회원 제외 → 버퍼 적재, NOOP 종료
...
│   └── schedule/BatchJobScheduler.kt                  # + @Scheduled(cron 외부화) 메서드 1개
├── src/main/resources/application.yml                 # kbap.batch.scan-suggestion.{cron,member-chunk-size,ttl} · kbap.push.expo.{concurrency,min-request-interval,retry.*}
