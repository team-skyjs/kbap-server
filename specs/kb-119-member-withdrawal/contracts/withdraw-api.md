...
| 400 | 회원 없음(이미 탈퇴 포함) | 해당 회원을 찾을 수 없습니다 |
...
1. `@AuthMemberId` 로 회원 해석 → `memberRepository.findById(memberId)`, 없으면 400.
...
| 호출 | 결과 |
|---|---|
| `GET /api/v1/members/me/profile` (기존 access token) | 400 — 해당 회원을 찾을 수 없습니다 |
