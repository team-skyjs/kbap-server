...
- [x] T044 KB-468 Jira 코멘트(quickstart §3 문구) + FE 에 `contracts/push-data-contract.md` 의 data 계약(MEAL_TIME 추가) 공유. **FE 요구사항 함께 전달**: 푸시 언어는 `notification_device.lang` 에만 의존하므로 앱 실행(포그라운드 진입)마다 + 기기 언어 변경 감지 시 `PUT /api/notifications/tokens` 를 현재 `lang` 으로 재호출해야 한다(재호출 전까지 이전 언어로 발송, 과거 알림함 행은 소급 안 됨). DoD 체크박스 갱신.
- [x] T045 `open-draft-pr-to-develop` 로 draft PR.
