...
>
> **2026-08-10 개정 2**: 무조건 nullable 완화 대신 **`X-App-Version` 헤더(선택)의 버전 값이 `1.1.0` 이상인지로 분기**한다(사용자 결정, research R1 개정 2). 미전송·미만·형식 오류면 두 필드 필수·누락 시 400 이라는 1.0.0 계약이 그대로 복원됐고, `1.1.0` 이상이면 서버가 랜덤 지정한다. 버전 파싱·비교는 `api.core.AppVersion`(단위 테스트 `AppVersionTest`). 계약·검증 시나리오는 [contracts/member-onboarding.md](./contracts/member-onboarding.md) 참조.
