...
## R1. 구버전(1.0.0) 앱과의 공존 수단 (2026-08-10 개정 3)

**Decision**: 온보딩 요청의 **`X-API-Version` 헤더(선택) — 클라이언트가 선언하는 계약 버전(정수, 기본 1)**로 분기한다. `2` 이상이면 `nickname`·`profileImageUrl` 을 무시하고 서버가 랜덤 지정하며, 미전송·`1`·파싱 불가면(1.0.0 앱) 두 필드 필수·누락 시 400 이라는 v1 계약이 그대로다. 버전 해석(`toIntOrNull ?: 1`)은 web 계층이 소유하고 도메인은 "null = 지정"만 안다. 별도 v2 엔드포인트는 열지 않는다.
...
- *`X-App-Version`(앱 빌드 semver) `>= 1.1.0` 비교* — 개정 2 결정. 플랫폼별 버전 번호 분기·서버의 앱 릴리스 의존 문제로 **같은 날 계약 버전 헤더로 대체**(semver 파싱 유틸 `AppVersion` 도 함께 제거).
