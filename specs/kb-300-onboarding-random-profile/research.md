...
## R1. 구버전(1.0.0) 앱과의 공존 수단 (2026-08-10 개정 4)

**Decision**: 온보딩 요청의 **`X-API-Version` 헤더(선택) — 클라이언트가 선언하는 계약 버전**으로 분기한다. 계약 버전 표기는 **캘린더 버저닝 `yyyy.mm.sprint차수`**(토스페이먼츠 날짜 버저닝을 스프린트 차수로 커스텀 — 비호환 변경만 새 버전을 릴리즈하고 하위 호환 변경은 버전을 올리지 않는다). 이번 온보딩 계약은 **`2026.08.07`** — 이상이면 `nickname`·`profileImageUrl` 을 무시하고 서버가 랜덤 지정하며, 미전송·이전 버전·파싱 불가면(1.0.0 앱) 두 필드 필수·누락 시 400 이라는 종전 계약이 그대로다. 버전 해석(`api.core.ApiVersion` — 3파트 파싱·숫자 비교)은 web 계층이 소유하고 도메인은 "null = 지정"만 안다. 별도 v2 엔드포인트는 열지 않는다.

**Rationale**: 엔드포인트 하나로 신·구가 공존하면서도, 구버전 계약이 문자 그대로 보존된다 — 구버전 앱의 필드 누락 버그가 조용히 랜덤 지정으로 흘러가지 않고 종전처럼 400 으로 드러난다. **앱 버전이 아니라 계약 버전인 이유**: 앱 버전은 "어떤 앱이 호출했나"라는 트래킹 관심사고, 동작 분기의 근거는 "클라이언트가 어떤 계약을 기대하나"다. 앱 버전에 묶으면 iOS/Android 릴리스 번호가 갈라질 때 임계값 의미가 흔들리고 서버가 앱 릴리스 일정을 알아야 한다. 헤더는 필수가 아니므로 클라이언트 협조 없이도 서버를 먼저 배포할 수 있다.
...
- *`X-App-Version`(앱 빌드 semver) `>= 1.1.0` 비교* — 개정 2 결정. 플랫폼별 버전 번호 분기·서버의 앱 릴리스 의존 문제로 **같은 날 계약 버전 헤더로 대체**.
- *계약 버전을 정수(1·2·…)로 표기* — 개정 3 결정. 동작은 같지만 팀 버저닝 정책을 캘린더 표기(`yyyy.mm.sprint차수`)로 정하면서 **같은 날 `2026.08.07` 표기로 대체** — 버전만 봐도 언제 릴리즈된 계약인지 드러난다.
- *헤더 존재 여부로만 분기(값 미해석)* — 이후 다른 목적으로 헤더를 싣는 순간 분기가 오작동한다. 기각.
