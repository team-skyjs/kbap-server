...
**Decision**: 온보딩 요청의 **`X-App-Version` 헤더(선택) 버전 값**으로 분기한다. `1.1.0` 이상이면(신버전 앱) `nickname`·`profileImageUrl` 을 무시하고 서버가 랜덤 지정하며, 미전송·`1.1.0` 미만·파싱 불가면(1.0.0 앱) 두 필드 필수·누락 시 400 이라는 종전 계약이 그대로다. 버전 해석(`api.core.AppVersion` — semver 파싱·숫자 비교)은 web 계층이 소유하고 도메인은 "null = 지정"만 안다. 별도 v2 엔드포인트는 열지 않는다.
...
- *헤더 존재 여부로만 분기(값 미해석)* — 파싱이 없어 더 단순하지만, 1.1.0 이전 버전이 이미 헤더를 싣고 있었거나 이후 다른 목적으로 헤더를 보내게 되면 분기가 오작동한다. 버전 임계값(`>= 1.1.0`) 비교로 확정 — 파싱 불가 값은 구버전 폴백이라 오타 헤더도 안전하다(사용자 결정).
