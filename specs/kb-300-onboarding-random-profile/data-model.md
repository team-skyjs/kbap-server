...
| 필드 | 타입 | X-API-Version >= 2026.08.07 | 미전송·이전·형식 오류 |
|------|------|-----------|-----------|
...
`MemberController` 가 `@RequestHeader("X-API-Version", required = false)` 를 받아 `ApiVersion.parseOrNull(헤더) >= 2026.08.07` 판정으로 `toInput(memberId, serverAssignsProfile)` 을 분기한다. `api.core.ApiVersion` 은 `yyyy.mm.sprint차수` 3파트 파싱(오타·파트 수 불일치는 null)과 숫자 비교만 담당한다. `serverAssignsProfile = true` 면 두 필드를 null 로 넘겨 `MemberService` 가 추첨으로 채우고, `false` 면 두 필드 null 시 `BusinessException(INVALID_REQUEST)` — Jackson 역직렬화 거절이 담당하던 종전 400 `COMMON-002` 를 동일 코드로 유지한다.
...
| 온보딩 완료 — 종전 계약(헤더 없음·이전 버전) | `true` | 클라이언트 전송값 | 클라이언트 전송값 |
| 온보딩 완료 — 2026.08.07 계약 이상 | `true` | 생성 코드(`K7M2XB`) | 후보 추첨값 |
