...
interface FoodCandidateRepository {
    ...
    // 승격 대상 조회 — DB 술어로 완성·미승격 후보만 id 오름차순 keyset 커서 조회(afterId 초과, afterId=0 이면 처음부터).
...
- `findPromotable` 은 `published_food_id IS NULL AND description IS NOT NULL AND JSON_LENGTH(description_translations)=9 AND JSON_LENGTH(substance_mapping)>0 AND id > :afterId` 를 만족하는 것만, `id` 오름차순 keyset 커서로 `size` 개 반환(offset 없음 — 실패분이 offset0 을 점유해 상위 완성분을 가리는 정체 방지).
...
interface FoodRepository {
    ...
    fun save(food: Food): Food                     // (추가) insert 전용, 저장된(id 포함) Food 반환
}
```

**계약**:
- `save` 는 **insert 전용**(업서트 아님) — 항상 새 `food` row 를 삽입하고 저장된(id 포함) Food 를 반환한다. 동명(korean_name) 중복 방지는 저장소가 아니라 **호출자(승격 잡)** 가 skip-link 로 책임진다.
- `food` 저장 시 연관 `food_avoidance_substance` 를 전달된 성분으로 함께 적재한다(정규화).
...
run():
  lastId = 0
  loop:
    batch = candidateRepo.findPromotable(lastId, SIZE)   # id > lastId 인 완성·미승격 후보
    if batch empty: break
    for c in batch:                      # 음식 1건 = 트랜잭션 1개
      lastId = c.id                      # 성공·실패·미완성 무관 커서 전진(정체 방지)
      if not c.isComplete(): continue    # SQL 선필터 통과분 도메인 재검증
      try (tx):
        existing = foods.findByKoreanName(c.koreanName)   # 동명 food 존재 확인
        foodId = existing?.id ?: foods.save(c.toFood()).id # 있으면 링크만(미수정), 없으면 insert
        candidateRepo.markPublished(c.id, foodId)
      catch: log, continue               # 실패 격리 — 나머지 진행, 실패분은 다음 실행(lastId=0)
```

**불변식**:
- 완성(`isComplete`) 아닌 candidate 는 절대 `food` 로 적재되지 않는다(FR-004/SC-002).
- 같은 korean_name 의 `food` 가 이미 있으면(시드 또는 이전 승격) **skip-link** — 기존 food 를 candidate 값으로 덮지 않고 그 `food.id` 로 링크만 한다(기존 food 불변). 없을 때만 insert 한다 → 중복 row 없음(SC-003).
