...
**Primary Dependencies**: Spring Boot 4.1, Spring Batch(기존 `:batch`), OpenAI text-embedding-3-small(Spring AI `OpenAiEmbeddingModel`, dimensions 옵션 명시 — `TextEmbeddingClient` seam, 2026-08-13 Bedrock Titan 에서 전환·AWS 의존 소거), mongodb-driver-sync(`:api` 개별 의존 → **`:common` 으로 승격** — 읽기·쓰기 공용)
