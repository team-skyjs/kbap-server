...
### User Story 3 - 관리자 수동 벡터 적재 (Priority: P3) — 2026-08-13 개정

기존 READY 음식은 자동 백필하지 않는다. 관리자가 기존 데이터를 랭체인 재수집으로 먼저 갱신한 뒤, 관리자 화면에서 **원하는 음식만 골라 수동으로 벡터 적재를 지시**한다(UPSERT 아웃박스 생성). 이후 신규 건은 READY 전이 이벤트가 증분 처리한다.

**Why this priority**: 기존 데이터는 랭체인 1회 재수집으로 콘텐츠(긴 설명 포함)를 최신화한 후 적재하는 것이 운영 계획이라, 배포 시점 자동 백필은 낡은 콘텐츠를 적재하는 역효과가 있다. 적재 시점 통제권을 관리자에게 둔다.

**Independent Test**: READY·활성 음식 N건 시드 → 관리자 적재 지시 → 대상 건만 UPSERT/PENDING 생성(비READY·삭제 음식 제외, 중복 억제) → 배치 실행 후 적재 확인.

**Acceptance Scenarios**:

1. **Given** 벡터 아웃박스가 없는 READY·활성 음식들, **When** 관리자가 벡터 적재를 지시하면, **Then** 대상 음식에 UPSERT/PENDING 아웃박스가 생성된다(1회 지시당 상한 존재).
2. **Given** 비READY·삭제 음식이 섞인 대상, **When** 적재를 지시하면, **Then** READY·활성 음식만 생성되고 나머지는 제외된다.
3. **Given** 이미 같은 작업의 PENDING 이 있는 음식, **When** 다시 지시하면, **Then** 중복 생성되지 않는다.
...
- **FR-009**: 관리자는 화면에서 기존 READY·활성 음식의 벡터 적재를 수동 지시할 수 있어야 한다 — 대상 필터(READY·활성)·1회 상한·중복 억제를 적용해 UPSERT/PENDING 아웃박스를 생성한다. (2026-08-13 개정 — 배포 시 자동 백필 폐기, 랭체인 재수집 후 수동 적재 운영)
...
- 임베딩 계약: 이름 + 긴 설명(long_description) 텍스트, 256차원, OpenAI text-embedding-3-small(2026-08-13 Bedrock Titan 에서 전환 — 팀 OpenAI 단일 벤더 정합, hash 의 model 필드로 기존 적재분 자동 재임베딩). 긴 설명 컬럼은 이미 존재한다.
