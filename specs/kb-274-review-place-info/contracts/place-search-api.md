...
리뷰 API 와 분리된 **별도 컨트롤러**(`com.kbap.api.place.PlaceController`). 서버가 카카오 로컬 키워드 검색을 대신 호출한다 — 클라이언트는 사용자 위치(위도·경도)를 보내며, `query` 가 없으면 서버가 키워드를 `음식점` 으로 고정해 주변 식당을, 있으면 해당 키워드로 검색한다(화면 진입 시 주변 탑10 → 사용자 검색 시 query 재호출). 응답은 `BaseResponse<T>` 봉투, 경로는 신규 리소스 규약(KB-321)에 따라 `ApiPaths.API` 기준.
...
| query | string | — | 검색 키워드(최대 100자). 없거나 공백이면 `음식점` 고정 |
