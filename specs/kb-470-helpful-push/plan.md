...
리뷰 좋아요(`POST /api/reviews/{reviewId}/like?liked=true`)가 **새 좋아요이고 작성자 본인이 아니면** `ReviewService.likeReview` 가 트랜잭션 안에서 `ReviewLiked(reviewId, authorMemberId, foodId)` 를 발행한다. api 의 `HelpfulPushListener` 가 `@Async @TransactionalEventListener(AFTER_COMMIT)` 로 받아 — 같은 리뷰의 최근 1시간 HELPFUL 알림함 행이 있으면 건너뛰고 — 음식 이름을 10개 언어로 만든 뒤 기존 공용 `PushHandler.send(PushRequest(HELPFUL, [author], argsByLang, data = {reviewId}))` 한 줄로 발송한다. Jira 의 아웃박스+batch 폴링은 채택하지 않는다(research §1). 공용 파이프라인 변경은 셋뿐: `NotificationType.channelId` 활동 유형 `"activity"`, `PushRequest.argsByLang`(기기 언어별 인자), `NotificationJpaRepository` 파생 쿼리 1개. 스키마 변경·신규 프로퍼티·신규 의존 없음.
...
**Testing**: api `@IntegrationTest`(컨텍스트 1개 — `FakePushSender` 기본 포함) `ReviewLikeControllerTest` 에 시나리오 추가, 비동기는 Kotest `eventually`/`continually` / common `PushDispatchServiceTest`(채널·언어별 인자)·`NotificationJpaRepositoryTest`(쿼리) / `ModuleBoundaryTest`(arch) 회귀.
...
**Scale/Scope**: 신규 api 2(`review.ReviewLiked`·`notification.HelpfulPushListener`) + api 변경 1(`ReviewService.likeReview`) + common 변경 4(`NotificationType`·`PushRequest`·`PushDispatchService`·`NotificationJpaRepository`) + 테스트 수정 3(`PushDispatchServiceTest`·`NotificationJpaRepositoryTest`·`ReviewLikeControllerTest`).

## Constitution Check

| 원칙 | 판정 | 근거 |
|------|------|------|
| I. Test-First | PASS | quickstart §1: 채널 → 언어별 인자 → 쿼리 → 발행 조건+리스너 통합 → 실패 격리. 각 단계 Red 확인 후 구현. |
...
| IV. Persistence Ownership | PASS | 묶음 판정은 리포지토리 파생 쿼리를 소비 계층(리스너)이 직접 호출 — KB-471 tasklet 의 "슬롯당 1회 상한" 과 같은 위치·수위. 위임 창구 서비스 없음. 트랜잭션: `likeReview` 명시 `@Transactional`, `prepare`/`record` 자체 `@Transactional`, 리스너는 무트랜잭션(의도적 — research §2). |
...
**Post-design re-check (Phase 1 후)**: PASS 유지. 신규 추상화 없음(이벤트 data class 1·리스너 1). `argsByLang` 은 기본값 있는 필드 추가라 기존 호출자(관리자 발송·배치) 무변경. 묶음 창은 상수(프로퍼티 아님).
...
├── spec.md
├── plan.md                    # 이 파일
├── research.md                # Phase 0 — 발송 주체·@Async+AFTER_COMMIT·발행 조건·묶음·언어별 인자·채널·관측·위치
├── data-model.md              # Phase 1 — 흐름별 읽기/쓰기·이벤트·값 타입 변경·쿼리·data JSON
...
└── NotificationJpaRepository.kt       # + findByMemberIdAndTypeAndCreatedAtAfter(memberId, type, since)
...
└── notification/
    └── HelpfulPushListener.kt         # 신규: @Async @TransactionalEventListener(AFTER_COMMIT) — 묶음 창(1h 상수) → 음식 이름 10개 언어 → PushHandler.send → try/catch 로그

common/src/test/kotlin/com/kbap/common/domain/notification/
├── PushDispatchServiceTest.kt         # 채널 단언 activity 로 교체 + given("언어별 인자")
└── NotificationJpaRepositoryTest.kt   # + 회원·type·since 경계 시나리오

api/src/test/kotlin/com/kbap/api/review/
└── ReviewLikeControllerTest.kt        # + given("좋아요 알림") — 발송/제외/묶음 시나리오(eventually·continually), 리스너 실패 격리
```
...
없음 — 헌법 위반 없음. 단순화 선택(묶음 창 상수·메모리 리뷰 매칭·Jira 아웃박스 기각)은 research 에 근거를 남겼다.
