...
- **완료 판정은 `wait services-stable` 대신 rolloutState 폴링**(리뷰 지적 반영): `aws ecs wait services-stable` 는 `length(deployments)==1 && running==desired` 에서만 성공하고 40회×15초=**10분 고정 상한** 뒤 실패한다. 네이티브 블루/그린은 bake 동안 blue+green **두 리비전을 의도적으로 유지**하므로(이 워크플로가 `--deployment-configuration` 을 안 넘겨 서비스 설정에 위임한 그 bake), bake>10분이면 정상 배포가 오탐된다. 대신 `describe-services` 로 **PRIMARY 배포의 `rolloutState`** 를 폴링해 `COMPLETED`=성공·`FAILED`=실패(롤백 포함)로 판정하고, 타임아웃은 `vars.DEPLOY_TIMEOUT_SECONDS`(기본 1800초=30분, bake+여유로 조정)로 둔다 — deployment-aware + bake 정렬 타임아웃.
