...
- **릴리스 버전**: 저장소 루트 `VERSION` 파일(단일 출처)로 선언되는 명시적 버전 문자열(예: `1.0.0`). 릴리스 준비 시 staging 브랜치에서만 bump 하며 staging(`-rc`)·prod 이미지 태그의 근원이 된다. main 의 `release: x.y.z` 머지 커밋 메시지는 기록용이고 권위는 이 파일이다.
...
- 릴리스 버전은 저장소 루트 `VERSION` 파일 하나로 관리하고 **릴리스 준비 시 staging 브랜치에서 사람이 bump 한다**(예: `1.0.0`→`1.1.0`) — develop 은 bump 하지 않아(dev 는 sha 배포) staging→develop 병합에서 VERSION 충돌이 없다. 같은 버전을 올리지 않고 재배포하면 태그가 덮어써짐을 팀이 인지한다.
