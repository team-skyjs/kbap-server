...
- [x] T004 마이그레이션 파일 생성(현재 로컬 시각으로 명명, 예: `date +%Y.%m.%d.%H.%M.%S`) — `api/src/main/resources/db/migration/V<timestamp>__push_notification_tables.sql`. 파일 상단에 한 줄 주석(용도·테이블 4개 이름)만 두고 DDL 은 각 스토리 태스크에서 추가
- [x] T005 `ModuleBoundaryTest.allowedDomainDeps` 에 `"notification" to emptySet()` 추가 — `api/src/test/kotlin/com/kbap/api/architecture/ModuleBoundaryTest.kt` (이 맵은 발견된 컨텍스트 집합과 정확 일치를 요구하므로 첫 엔티티가 생기는 순간 Red 가 된다 → 선반영)
...
**Goal**: `notification_device` — 기기 설치 식별자 유일, 회원 nullable 연결·해제, 재등록 갱신, 회원 기준 다기기 조회, 게스트 광고성 동의 컬럼 3개(`updateMarketing` 전이).

**Independent Test**: `NotificationDeviceJpaRepositoryTest` 만으로 spec US1 시나리오 #1~#6 과 US2 #5(게스트 동의 전이) 검증. api 통합 컨텍스트 기동으로 DDL 정합 검증.

### Tests for User Story 1 (Test-First — 먼저 작성, 실패 확인) ⚠️

- [x] T006 [US1] `NotificationDeviceJpaRepositoryTest`(BehaviorSpec, `@SpringBootTest` + `@Import(MySqlContainerConfig::class)`) 작성 — `common/src/test/kotlin/com/kbap/common/domain/notification/NotificationDeviceJpaRepositoryTest.kt`. given/when/then(한국어): ① `register` 저장 후 `findByInstallationId` 로 1건, memberId null ② 같은 installationId 로 두 번째 `save` 시 `DataIntegrityViolationException`(유니크) ③ `linkMember` → `unlinkMember` 후 토큰·lang·marketing 컬럼 유지 ④ `renew` 로 expoToken 교체 시 id 불변 ⑤ 회원 하나에 기기 2건 → `findByMemberId` 2건 ⑥ `updateMarketing(true, "v2", now)` 후 재조회 시 marketing true·버전 v2·optInAt == now, `updateMarketing(false, null, later)` 후 둘 다 null ⑦ `delete()` 후 `findByInstallationId` null(소프트삭제 필터). 실행: `./gradlew :common:test` → 컴파일 실패(Red) 확인

### Implementation for User Story 1

- [x] T007 [US1] `NotificationDevice` 엔티티 작성 — `common/src/main/kotlin/com/kbap/common/domain/notification/model/NotificationDevice.kt`. `@Entity @Table(name = "notification_device", uniqueConstraints = [UniqueConstraint(name = "uk_notification_device_installation", columnNames = ["installation_id"])])`, `BaseEntity` 상속. 컬럼: `installation_id` VARCHAR(36) NOT NULL, `member_id` BIGINT NULL, `expo_token` VARCHAR(255) NOT NULL, `platform` `@Enumerated(STRING)` `columnDefinition = "ENUM('IOS','ANDROID')"`, `lang` VARCHAR(10) NOT NULL, `marketing` Boolean NOT NULL 기본 false, `marketing_consent_version` VARCHAR(20) NULL, `marketing_opt_in_at` `LocalDateTime?`. 메서드: `linkMember(memberId)`, `unlinkMember()`, `renew(expoToken, platform, lang)`, `updateMarketing(enabled, consentVersion, now)`, `isMarketingAllowed(requiredVersion)`, companion `register(...)`. 전 필드 기본값(no-arg 생성), 주석 없음
- [x] T008 [US1] `NotificationDeviceJpaRepository : JpaRepository<NotificationDevice, Long>` 작성 — `common/src/main/kotlin/com/kbap/common/domain/notification/NotificationDeviceJpaRepository.kt`. `findByInstallationId(installationId: String): NotificationDevice?`, `findByMemberId(memberId: Long): List<NotificationDevice>`, `findByExpoToken(expoToken: String): List<NotificationDevice>`. `./gradlew :common:test` Green 확인
- [x] T009 [US1] 마이그레이션에 `notification_device` DDL 추가 — T004 파일. `community_post` 스타일(소문자, 백틱 없음): id BIGINT AUTO_INCREMENT PK, installation_id VARCHAR(36) NOT NULL, member_id BIGINT NULL, expo_token VARCHAR(255) NOT NULL, platform ENUM('IOS','ANDROID') NOT NULL, lang VARCHAR(10) NOT NULL, marketing BOOLEAN NOT NULL DEFAULT FALSE, marketing_consent_version VARCHAR(20) NULL, marketing_opt_in_at DATETIME(6) NULL, status ENUM('ACTIVE','DELETED') NOT NULL DEFAULT 'ACTIVE', created_at/updated_at DATETIME(6) NOT NULL, `UNIQUE KEY uk_notification_device_installation (installation_id)`, `KEY idx_notification_device_member (member_id)`, `CONSTRAINT fk_notification_device_member FOREIGN KEY (member_id) REFERENCES member (id)`
- [x] T010 [US1] `TestTables.tables` 에 `"notification_device"` 추가(FK 자식인 `notification_dispatch`·`notification` 보다 뒤, `member` 보다 앞) — `api/src/test/kotlin/com/kbap/api/TestTables.kt`. `./gradlew :api:test` 로 Flyway 적용 + `ddl-auto=validate` 통과 확인(엔티티 컬럼 길이·타입이 DDL 과 어긋나면 여기서 Red)
