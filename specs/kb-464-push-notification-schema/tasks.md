...
- [ ] T003 [P] `NotificationPreferences(helpful = true, reviewReminder = true, marketing = false, marketingConsentVersion: String? = null, marketingOptInAt: LocalDateTime? = null)` data class + `DEFAULT` 작성(설정 응답·승계 조립용, JSON 매핑 없음) — `common/src/main/kotlin/com/kbap/common/domain/notification/model/NotificationPreferences.kt`
