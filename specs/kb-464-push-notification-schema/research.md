...
- **Alternatives considered**: 두 테이블 상태 컬럼(초안) — 위 결함. 기기 테이블에서만 제거(게스트 동의 미저장) — 게스트 넛지 포기 + 철회 증빙 소실은 그대로. 회원/게스트 테이블 분리 — NULL 은 없어지지만 승계가 "닫고 새로 만들기" 두 문장이 되고 한 사람의 이력이 두 곳에 갈림. `member_id` 는 NULL 허용이어도 FK 가 걸려(값이 있으면 실존 회원 강제) 무결성을 잃지 않는다. 상태 컬럼 + 이력 테이블 이중 쓰기 — 정본 둘 문제의 재현. 파생 캐시 컬럼 — 측정 없이 이중 쓰기 회귀라 기각.

## R5. 광고성 동의 계약과 보류 사항 (2026-09-07 FE 협의)

- **Decision**: 동의 스위치 이름은 알림 유형(NUDGE)이 아니라 법적 카테고리 **`marketing`**. API 계약 `marketing`·`marketingOptInAt`·`marketingConsentVersion`(**정수**, 구 문구 = 1). FE 내부 토글명 nudge 는 어댑터 매핑. 알림 유형 `NUDGE` 는 그대로.
- **Decision**: 동의 문구 버전은 `SMALLINT UNSIGNED` 정수. 문자열 비교는 `"v10" < "v2"` 함정이 있어 금지.
- **Decision**: 야간(21~08시) 전송 별도 동의는 **보류** — 발송 배치(KB-471)가 08~21 KST 밖에서는 보내지 않는 하드 가드. 저녁 넛지가 기획되면 원장에 `kind` 컬럼 추가.
- **Rationale**: 정보통신망법 50조 — 광고성 정보는 사전 동의·동의 증빙·야간 별도 동의가 필요. 현재 FE 문구는 카테고리 동의가 아니라 점심 넛지 한정 동의라, 문구를 바꾸는 FE 작업과 버전이 같이 가야 구 동의자를 구분할 수 있다.
