...
- **첫/마지막 리뷰 판정**: 같은 트랜잭션 안에서 `countByMemberIdAndFoodId` (작성: 저장 후 count==1 → uniqueDelta=1 / 삭제: 삭제 후 count==0 → uniqueDelta=1). 동시성은 **카운트 UPDATE 선행**으로 직렬화한다(2026-07-30 Codex 리뷰 반영·감수 결정 폐기, 별도 FOR UPDATE 조회는 사용자 리뷰로 제거) — `increaseReviewCount` UPDATE 가 회원 행 X-lock 을 잡아 커밋까지 유지하므로, 첫/마지막 판정 count 를 그 뒤에 두면 같은 회원의 동시 작성/삭제가 자연 직렬화된다. 작성 시 UPDATE 는 INSERT 보다 먼저 실행한다 — INSERT 의 FK 검증(fk_food_review_member)이 member 행 S-lock 을 선점한 뒤 UPDATE 가 X-lock 을 요구하면 동시 작성 간 데드락이 난다(실측). 증감은 컬럼별 단일 메서드 4개(review_count·unique 각 증/감, 감소는 하한 가드). 판정 트랜잭션은 READ_COMMITTED 명시 — RR 스냅샷이 락 대기 전에 고정되면 count 가 과거를 본다. 수정/삭제 대상 리뷰 행도 X-lock 조회(`ReviewJpaRepository.findByIdForUpdate`)로 읽는다 — 같은 리뷰 동시 삭제의 중복 차감, 수정의 stale ACTIVE flush 가 삭제를 되살리는 경합 차단(2026-07-30 Codex 2차 리뷰 반영).
