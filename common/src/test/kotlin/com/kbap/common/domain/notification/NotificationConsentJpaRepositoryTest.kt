...
class NotificationConsentJpaRepositoryTest : BehaviorSpec() {
    ...
    init {
        ...
        given("회원 동의") {
            `when`("문구 버전 2 로 동의하면") {
                clear()
                repository.save(NotificationConsent.grantForMember(memberId = 1L, installationId = "inst-1", consentVersion = 2, now = now))
            ...
            `when`("동의를 철회하면") {
                clear()
                val saved = repository.save(NotificationConsent.grantForMember(1L, "inst-1", 2, now))
            ...
            `when`("열린 동의가 두 건 생긴 상태에서 회원 기준으로 전부 닫으면") {
                clear()
                repository.save(NotificationConsent.grantForMember(1L, "inst-1", 2, now))
                repository.save(NotificationConsent.grantForMember(1L, "inst-2", 2, now))
                val closed = transactionTemplate.execute { repository.closeOpenByMemberId(1L, now.plusDays(1)) }

                then("두 건 모두 닫혀 자가 치유된다") {
                    closed shouldBe 2
                    repository.findOpenByMemberId(1L).shouldBeEmpty()
                }
            }
            ...
            `when`("이미 철회된 동의를 다시 철회하면") {
                val consent = NotificationConsent.grantForMember(1L, "inst-1", 2, now)
        ...
        given("발송 가능 판정") {
            `when`("요구 버전이 2 일 때") {
                val v2 = NotificationConsent.grantForMember(1L, "inst-1", 2, now)
                val v10 = NotificationConsent.grantForMember(1L, "inst-1", 10, now)
                val v1 = NotificationConsent.grantForMember(1L, "inst-1", 1, now)
                val revoked = NotificationConsent.grantForMember(1L, "inst-1", 2, now).apply { revoke(now.plusDays(1)) }
        ...
        given("게스트 동의와 로그인 인수") {
            `when`("게스트 기기가 동의하면") {
                clear()
                repository.save(NotificationConsent.grantForInstallation("inst-g", 2, now))
            ...
            `when`("그 기기에서 회원이 로그인해 동의를 인수하면") {
                clear()
                val guest = repository.save(NotificationConsent.grantForInstallation("inst-g", 2, now))
                guest.claim(7L)
            ...
            `when`("이미 회원 동의인 행을 다시 인수하려 하면") {
                val consent = NotificationConsent.grantForMember(7L, "inst-g", 2, now)
