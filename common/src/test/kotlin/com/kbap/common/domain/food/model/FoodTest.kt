...
class FoodTest : BehaviorSpec({
    ...
    fun create(
        ...
        ingredients: List<FoodIngredient> = listOf(item("SOYBEAN", 100)),
    ) = Food(
    ...
    given("FoodIngredient.riskLevel — 포함 확률로 위험도 산출") {
    ...
    given("Food.ingredientsByProbability") {
        `when`("포함 성분이 포함 확률 내림차순이 아닌 순서로 담겨 있으면") {
            then("저장 순서와 무관하게 포함 확률 내림차순으로 정렬된 성분을 반환한다") {
                ...
                food.ingredientsByProbability().map { it.inclusionPercent } shouldBe listOf(100, 80, 50)
                food.ingredientsByProbability().map { it.code } shouldBe listOf("SOYBEAN", "TOFU", "CLAM")
            }
        }

        `when`("포함하는 기피 성분이 하나도 없으면") {
            then("빈 목록을 반환하고 음식은 유효하다") {
                val food = create(ingredients = emptyList())

                food.ingredientsByProbability() shouldBe emptyList()
                food.koreanName shouldBe "된장찌개"
            }
        }
    }

    // KB-301: 콘텐츠 채움용 갱신 메서드가 kbap-langchain 이관으로 비활성이다. 최종 삭제는 KB-302.
    ...
    given("Food 콘텐츠 완성 상태") {
        ...
        `when`("스캔 미스로 미완성 음식을 만들면") {
            then("한국어명만 갖고 FAILED 상태이며 성분은 미조사(null)다") {
                val food = Food.failed("우주라면")

                food.koreanName shouldBe "우주라면"
                food.contentStatus shouldBe FoodContentStatus.FAILED
                food.isReady() shouldBe false
                food.ingredients shouldBe null
            }
        }

        `when`("미완성 음식의 위험도를 물으면") {
            then("성분이 비어도 SAFE 가 아니라 UNKNOWN 이다") {
                Food.failed("우주라면").overallRisk(emptySet()) shouldBe RiskLevel.UNKNOWN
            }
        }

        `when`("한국어명이 blank 이면") {
            then("예외를 던진다") {
                shouldThrow<IllegalArgumentException> { Food.failed(" ") }
            }
        }
    }
        ...
        `when`("컬럼 길이(255)를 넘는 이름이면") {
            then("도메인이 거절한다(영속 계층 truncation 방지 최후 방어선)") {
                shouldThrow<IllegalArgumentException> {
                    Food.failed("가".repeat(KoreanMenuNameNormalizer.MAX_MENU_NAME_LENGTH + 1))
                }
            }
        }
    }
    ...
    given("Food.incomplete — 표시명(원본 표기) 보존") {
        `when`("매칭용 정규화명과 원본 표기를 함께 주면") {
            then("두 이름을 각각 보존한다") {
                val food = Food.failed("들깨칼국수", "들깨 칼국수")
        ...
        `when`("원본 표기를 생략하면") {
            then("매칭용 이름을 표시명으로 쓴다") {
                Food.failed("우주라면").displayName shouldBe "우주라면"
            }
        }

        `when`("표시명이 blank 이면") {
            then("예외를 던진다") {
                shouldThrow<IllegalArgumentException> { Food.failed("우주라면", " ") }
            }
        }
        ...
        `when`("표시명이 컬럼 길이(255)를 넘으면") {
            then("도메인이 거절한다") {
                shouldThrow<IllegalArgumentException> {
                    Food.failed("우주라면", "가".repeat(KoreanMenuNameNormalizer.MAX_MENU_NAME_LENGTH + 1))
                }
            }
        }
    }
