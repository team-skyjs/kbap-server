...
class ReviewJpaRepositoryTest : BehaviorSpec() {
    ...
    init {
        ...
        fun seedFoodRow(foodId: Long): Unit =
            dataSource.connection.use { c ->
                c.prepareStatement(
                    """
                    ...
                    ps.setLong(1, foodId)
                    ps.setString(2, "레포음식$foodId")
                    ps.setString(3, "레포음식$foodId")
                    ps.executeUpdate()
                }
            }
        ...
        given("findReviewPage — 차단 회원 제외 필터") {
            val foodId = 750L
        ...
        given("findReviewPage — foodId null 전체 목록 keyset") {
        ...
        given("findReviewPage — foodId null 전체 목록 제외 규칙") {
            fun saveFood(koreanName: String): Food = foodJpaRepository.save(Food(koreanName = koreanName))
