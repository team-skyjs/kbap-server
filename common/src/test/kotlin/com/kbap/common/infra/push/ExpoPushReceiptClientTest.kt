...
class ExpoPushReceiptClientTest : BehaviorSpec({
    fun fixture(accessToken: String = ""): Pair<ExpoPushReceiptClient, MockRestServiceServer> {
        val builder = RestClient.builder()
        val server = MockRestServiceServer.bindTo(builder).build()
        return ExpoPushReceiptClient.create(BASE_URL, accessToken, builder) to server
    }

    given("Expo 영수증 조회 어댑터") {
        `when`("ok·코드 있는 오류·코드 없는 오류가 섞여 오고 한 건은 응답에 없으면") {
            then("있는 영수증만 계약대로 옮기고 없는 id 는 결과에서 빠진다") {
                val (receiptClient, server) = fixture("secret")
                ...
                val receipts = receiptClient.fetch(listOf("a", "b", "c", "d"))
        ...
        `when`("1,500건을 조회하면") {
            then("1000·500 두 요청으로 나눈다") {
                val (receiptClient, server) = fixture()
                server.expect(requestTo(RECEIPTS_URL))
                ...
                receiptClient.fetch(List(1500) { "t$it" }).keys shouldBe setOf("t0", "t1000")
                server.verify()
            }
        }
        ...
        `when`("조회할 id 가 없으면") {
            then("요청하지 않는다") {
                ...
                receiptClient.fetch(emptyList()) shouldHaveSize 0
                server.verify()
            }
        }
        ...
        `when`("Expo 가 5xx 로 답하면") {
            then("예외를 그대로 올린다") {
                ...
                shouldThrow<RestClientException> { receiptClient.fetch(listOf("a")) }
            }
        }
    }
})
