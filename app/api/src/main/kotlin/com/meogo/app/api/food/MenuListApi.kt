...
@Tag(name = "음식 목록", description = "검색어 없이 최신순 메뉴를 keyset 커서로 조회하는 API")
interface MenuListApi {
    @Operation(
        summary = "메뉴 목록 조회 (무한 스크롤, no-offset)",
        description = """
            검색어 없이 전체 메뉴를 최신 등록순(foodId 내림차순)으로 한 페이지 20개씩 조회한다.
    ...
    @ApiResponses(
        value = [
            ApiResponse(responseCode = "200", description = "조회 성공 — 최신순 메뉴 요약(≤20)·nextCursor·hasNext 반환"),
