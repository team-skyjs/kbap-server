...
`specs/kb-90-menu-name-refinement/plan.md` (KB-90 메뉴 스캔 정제·매칭·위험도 — 흐름: **정규화(NFC→한글만, 빈 키=메뉴 아님) → 비지 않은 전 항목을 Upstage `solar-pro` 동기 1콜(같은 길이 배열 + `NOT_FOOD` 센티넬, temperature 0) → `findByKoreanMatchKeys` 배치 1쿼리 매칭 → 위험도 산출**. miss 는 별도 대기열 없이 **`food` 에 `content_status=INCOMPLETE` 로 in-place 등록**(get-or-create)하고, 완성(READY) 전까지 목록·상세에 노출 안 함(serving gate는 목록 JPQL·검색 네이티브 쿼리 + 상세 어댑터에만 — 스코어링 배치가 쓰는 공유 쿼리엔 미적용, 안 그러면 미완성 음식이 영영 안 채워짐). **`Food.overallRisk()` 가 `!isReady()` 면 `UNKNOWN` 강제** — `RiskLevel.aggregate(빈 목록)=SAFE` 함정을 도메인에서 차단. 위험도는 유스케이스가 `Food.overallRisk(회피코드)` 직접 호출(더미 회피성분 제공자, 카탈로그 미조회 — KB-62 와 동일 규칙). **폴백**(interpreter 미구성·예외·타임아웃·응답 개수 불일치): 정규화 exact 매치만 하고 **미완성 food 를 만들지 않으며**(테이블 오염 방지) 응답에 `degraded=true`. **스캔 내역 미저장**(`menu_scan`·`scanned_menu_item` DROP, `MenuScan` 애그리거트 제거), **요청에서 바운딩박스 미수신**(itemId 중복 400 은 요청 DTO `@AssertTrue` 로 이관). 응답 = `{degraded, results[]{itemId, matchStatus(MATCHED|UNMATCHED), foodId?, riskLevel}}` — **비음식은 결과에서 제외**되어 개수가 요청보다 적을 수 있고 클라이언트는 `itemId` 로 짝을 맞춘다. Flyway `food.korean_match_key` 생성 컬럼은 **`COLLATE utf8mb4_bin` 필수**(기본 collation 에선 `[^가-힣]` 범위 오류로 마이그레이션 실패 — Testcontainers 가 못 잡음). 실제 solar-pro 스모크로 오탈자 교정·비음식 제외·미완성 등록·serving gate·dedup 검증 완료).
<!-- SPECKIT END -->
