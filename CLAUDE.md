...
`specs/kb-103-member-domain/plan.md` (KB-103 회원 도메인 — 소셜 신원·프로필·온보딩 상태·탈퇴(soft delete). 빈 `:core:member` 에 회원 애그리거트(Member 1—N SocialIdentity·MemberProfile·OnboardingStatus PENDING/COMPLETED)와 신원 해소 도메인 서비스 `MemberIdentityResolver` 를 만들고, `:infra:persistence` 에 JPA 어댑터(`members`·`member_social_identities`)·Flyway 마이그레이션을 얹는 2계층 슬라이스(web/application 변경 없음 — 소비자는 KB-102 로그인·KB-104 온보딩). **신원 해소 키는 (provider, providerUserId) 단독 — email 자동 통합 철회(사용자 결정 2026-07-08)**, email 은 참고 정보(nullable·인덱스 없음)이며 JWT 클레임에도 미포함. 동시 중복 가입은 DB UNIQUE(provider, provider_user_id) + 어댑터 예외 번역(DUPLICATE_SOCIAL_IDENTITY) + resolver 재조회 1회로 수렴. 탈퇴 = 회원 soft delete + 신원 hard delete(PII 제거·재가입 유니크 충돌 해소, 재로그인 = 신규 가입). 기피성분 코드는 member 자체 `AvoidanceSubstanceCodeRef` 값타입(avoidance enum import 금지, 원칙 II)으로 `members.avoidance_substance_codes` JSON 컬럼 저장. `MEMBER` 는 MySQL 8.0 예약어라 테이블명 복수형. 테스트: 도메인 BehaviorSpec 단위(+fake repo 로 resolver 신규/기존/race/email 무관) + Testcontainers 어댑터 통합, 마이그레이션은 로컬 docker MySQL 실측.)
<!-- SPECKIT END -->
