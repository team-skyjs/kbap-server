...
`specs/kb-172-deploy-automation/plan.md` (KB-172 브랜치별 배포 자동화 — 수동 배포(로컬 빌드→ECR push→EC2 접속 docker 명령)를 브랜치 푸시만으로 자동화. 환경별 워크플로 3파일: `deploy-dev.yml`(develop→공용 EC2 `api-dev` :8080)·`deploy-staging.yml`(staging→`api-staging` :8081)·`deploy-prod.yml`(main→ECS 네이티브 블루/그린, CodeDeploy 미사용 — 사용자 확인 07-20). 공통 흐름: 기존 멀티스테이지 Dockerfile 로 linux/amd64 빌드→ECR `kbap-api` push(**태그 환경별** — dev=git sha, staging=`$(cat VERSION)-rc`, prod=`$(cat VERSION)`; 단일 출처 루트 `VERSION` 파일 1줄, 릴리스마다 커밋 bump, `latest` 미사용 — 같은 태그 공유 시 prod 빌드가 staging 검증 이미지를 덮어써 `-rc` 로 분리)→dev/staging 은 SSM Run Command(AWS-RunShellScript)로 pull/stop/run(--env-file /opt/kbap/*.env — 비밀은 호스트 소유)+**스크립트 내 헬스체크 루프**(실패=명령 실패=워크플로 실패), prod 는 describe-services→describe-task-definition→이미지만 교체 리비전 등록→`ecs update-service --task-definition`(블루/그린 트리거)→`ecs wait services-stable`. 블루/그린 전략·타깃그룹·bake time 은 ECS 서비스 사전 구성(인프라 소유), `--deployment-configuration` 미전달(bake·서킷브레이커 덮어쓰기 방지). 인증: GitHub OIDC + 환경별 IAM 역할 3개(gha-deploy-dev/staging/prod), 신뢰 정책 `sub`=`repo:…:environment:<env>` 로 교차 배포 IAM 차단. 값은 GitHub Environments(dev/staging/prod) **variables**(secret 0개), prod 승인 게이트는 미설정(추후 protection rule 만). 동시성 `concurrency: deploy-<env>, cancel-in-progress: false`(마지막 푸시=최종 상태). 롤백: 세 워크플로 공통 `workflow_dispatch`+`image_tag` 입력=빌드 생략 재배포(dev=이전 sha, staging/prod=이전 버전). reusable workflow·taskdef/appspec 저장소 반입·gradle 별도 빌드·buildx 캐시 전부 기각(YAGNI). **프로덕션 코드·설정·DB 0줄** — JVM 테스트 표면 없음(KB-169 선례), 검증은 actionlint+환경별 실배포 1회 런북(quickstart §6). AWS 측 구성(OIDC provider·IAM·EC2 env-file·Environments·staging 브랜치 생성)은 quickstart 런북. data-model·contracts 없음, :app:batch 배포 범위 밖.)
