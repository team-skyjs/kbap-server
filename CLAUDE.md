...
`specs/kb-93-llm-scoring-cost-reduction/plan.md` (KB-93 LLM 스코어링 호출 비용 절감 — 청크당·모델당 ₩1 미만(실측 OpenAI ≈₩10·Gemini ≈₩6.3·Upstage ≈₩0.9, 출력의 80%+ 가 음식 설명). **산출물·정확도 불변**(앙상블 공식·3모델 전량 취합 확정·`inclusionConfidence` 1~100 은 KB-53 그대로), 비용만 낮춘다. 세 갈래: (1) **포맷 압축** — 프롬프트에 음식·후보 성분을 인덱스 열거(후보는 코드만·라벨 제거), 응답은 `{"c":[커버 음식 인덱스],"r":[[음식idx,성분idx,score,prob],...]}` 압축 JSON(`c`=커버리지 attest — 미포함 쌍 생략 포맷에서 KB-53 커버리지 게이트 보존), (2) **텍스트 역할 분리** — 음식 설명은 전 모델 프롬프트에서 완전 제거(별도 티켓 이관), 음식명 9언어 번역은 **Gemini 1개 모델**의 같은 1회 호출에만 `t`=[음식idx,[9개 번역—고정 언어 순서·키 생략]] 로 수신(청크당 모델별 호출 1회 유지·번역 담당은 배치 조율만 인지, `:core:research` 는 `includeNameTranslations` 불리언만), (3) **모델·옵션 튜닝** — gpt-5-nano + `reasoning-effort: minimal` + 전 모델 `max-output-tokens`(벤더별 매핑: openai→maxCompletionTokens·upstage→maxTokens·gemini→maxOutputTokens), [정적 system(지시문+후보 81종) → 동적 user(음식 청크)] 순서로 프리픽스 캐싱 정렬(자동 캐싱만, 캐싱은 추가 마진). `LlmFanoutClient` 에 모델별 요청 seam `generate((LlmModelId) -> LlmChatRequest)` 추가(벤더 중립). 파서는 **제자리 교체 + 골든 동등성 테스트**(KB-53 판단 규칙 의미 보존 — 항목 스킵·중복 첫-채택·커버리지 미달 미취합). `FoodContentSelector`·`ModelScoring` 형태 유지(설명은 항상 null 안전 산출). 비용 실측은 기존 토큰/비용 로그(SC-001). 위험도(KB-9)·영속(KB-54)·설명 생성(별도 티켓)은 범위 밖).
<!-- SPECKIT END -->
