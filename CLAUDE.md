...
`specs/kb-163-flyway-squash/plan.md` (KB-163 Flyway 마이그레이션 스쿼시 — 기존 22개(스키마+데모 시드 혼재)를 최종 상태 기준으로 재편: **스키마 전용 init 1개 + 마스터 시드(기피물질 카탈로그 81종)** 는 `db/migration`(전 환경), **데모 시드(음식 10건·food_avoidance_substance 매핑)** 는 `db/seed`(local·dev 만)로 분리, `spring.flyway.locations` 프로필별 분기(base=migration 만 — prod 안전 기본값, local·dev=+seed, 테스트=base 상속·자체 시드 원칙). 최종 스키마·데모 데이터는 docker MySQL 에 구 22개 적용 후 mysqldump 로 도출(신구 스키마 diff=0 검증). **핵심 판단: 홈서버 dev DB 는 Jira 원안(drop 재생성) 대신 flyway_schema_history 재기준선(baseline-version=새 파일 최고 버전)으로 회원·음식 데이터 손실 0 전환**(quickstart §4 런북). Test-First: 신규 `MigrationLayoutTest`(리소스 가드 — init 무INSERT·마스터 81건·db/migration 에 INSERT INTO food 부재)가 Red 진입점 + `AvoidanceCatalogSeedSyncTest` 경로 갱신(파일명 결합 주의사항). 신규 코드 0줄 — SQL 3개·yml 3곳·테스트 2개·삭제 22개. 도메인 모듈·:app:batch(flyway off) 범위 밖.)
