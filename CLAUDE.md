...
`specs/kb-171-storage-key-prefix/plan.md` (KB-171 이미지 업로드 객체 키 환경 접두(key-prefix) 지원 — prod S3 버킷을 전 환경이 공유하므로 업로드 API 경유 이미지(메뉴판 스캔·프로필)의 객체 키에 환경별 최상위 폴더 접두(`dev/`·`staging/`, prod·local 은 빈 값=기존 구조 유지)를 붙인다. `kbap.storage.key-prefix`(env `STORAGE_KEY_PREFIX`, 기본 빈 값) → `ImageUploadConfig` `@Value` 주입 → `ImageUploadProperties.keyPrefix` → `ImageUploadApplicationService.objectKey()` 에서 `trim('/')` 정규화 후 `"$prefix/$key"` 결합(빈 값이면 무접두 — `dev`/`dev/`/`/dev` 동일 결과, 선행·중복 슬래시 구조적 불가). yml 은 base `application.yml` 에 `${STORAGE_KEY_PREFIX:}` 선언 + dev·staging 프로필 동일 선언(가시성, Jira DoD), prod·local 미선언. 접두 포함 키가 그대로 DB ref 저장 → `ImageUrls.resolve` 조립 무변경, API 계약·DB 스키마·Flyway 0, 음식 사진(업로드 API 미경유 공유 자산)·IAM 권한 제한(인프라)·:app:batch 범위 밖. Test-First: `ImageUploadApplicationServiceTest` 에 접두 유무·슬래시 변형 시나리오 Red 먼저 — 프로덕션 3파일+yml 3파일 수정, 신규 파일 0.)
