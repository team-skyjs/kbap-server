...
- **LLM: Spring AI 2.0**(Boot 4 호환 라인) — 전용 모듈 **`:infra:llm`**(ADR-0010)에 `spring-ai-starter-model-openai` + `spring-ai-starter-model-bedrock`. 공개 API `LlmModelCaller`·값타입·구성이 이 모듈에 응집되고 **`:batch`가 `implementation`으로 직접 의존**해 잡에서 호출(seam 은 `:common` 의 `common.port.llm`). **채팅·비전은 OpenAI 단일 벤더 `gpt-5.6-luna` 로 통일**(KB-320 — Upstage·Gemini 와 다중모델 fan-out `LlmFanoutClient`·`LlmModelId` 는 제거)하고 `kbap.llm.*` 프로퍼티 + `@ConditionalOnProperty`로 명시 구성한다: `openai`(배치 콘텐츠 생성 + `avoidance` 오버라이드)·`vision`(메뉴판 스캔)·`image`(`gpt-image-2` 배치 이미지 생성)·`embedding`(Bedrock Titan). 키/활성 플래그가 없으면 caller 빈이 미생성돼 batch/web 부팅이 안전하며(단 api 는 `vision.enabled: true` 고정 — 스캔이 필수 기능이라 `OPENAI_API_KEY` 없이는 부팅하지 않는다. vision 전용 환경변수는 없다 — 사진 fetch 도메인은 `kbap.storage.public-base-url` 을 참조), Spring AI 자동구성 유입은 `application.yml`의 `spring.ai.model.*=none`으로 차단. 기피성분 조사는 단일 호출이며 응답이 계약(후보 코드·포함률·맵기 범위)을 어기면 종합하지 않고 예외 — 페이크 caller 로 단위검증(헌법 I).
