...
`specs/kb-53-llm-avoidance-scoring/plan.md` (KB-53 기피성분 포함 신뢰도 LLM 스코어링. 조사 대기열 음식을 **10개 청크**로 3개 LLM(KB-49 `:infra:llm` fan-out) 병렬 호출 → 각 모델이 대표 레시피 기준 **"포함된 성분만"** `score(0/1/2)·probability(1~100)` JSON 응답 → **Consensus Ensemble 공식**(참고 문서 §4: `base=0.6·(avg_score/2)+0.4·(avg_probability/100)` × agreement_factor 1.0/0.9/0.75, round·100)으로 (음식,성분)별 최종 **`inclusionConfidence` 정수 1~100** 종합. **스코어링 도메인 로직(프롬프트·파싱·앙상블)은 `:core:research` 순수 서비스**(ADR-0004, food/avoidance 타입 미import·primitive 값타입으로 격리), `:infra:llm` 무변경(벤더 중립), **조율은 `:app:batch` 잡**(ADR-0010/KB-49 배치-직접 — 컨텍스트 조합을 application 아닌 배치에서 하는 원칙 II 예외, 승격 트리거 ADR-0004 §6). 누락=미포함(score0/prob1 정규화)로 후보 81 전부 산출. **3개 모델 결과가 모두 취합돼야 확정**(부분 집계 금지) — 일부 실패(API 다운 등)는 모델별 별도 로깅 + 청크 미확정(재조사). **같은 호출에서 음식명 9개 언어 번역 + 음식 설명(ko 생성 + 9개 번역, 공백 포함 ≤200자, 헌법 V)도 JSON 수신**(`food.name_translations`·`description(_translations)`·`LocalizedText` 동형, `ko` 제외) — 텍스트는 앙상블 아님·우선순위 단일 모델 채택(`FoodContentSelector`). 음식 공급은 `:core:food` `FoodScoringSource` seam(초기=active food 읽기, 전용 대기열 테이블 후속). **위험도 판정(KB-9)·DB 영속(KB-54)은 범위 밖** — KB-53 은 산출까지. 프롬프트/파싱/앙상블 순수 단위검증(헌법 I, 골든 §5=74)).
<!-- SPECKIT END -->
