...
`specs/kb-123-member-ranking/plan.md` (KB-123 회원 랭킹 — 활동량을 점수로 환산해 7단계 등급으로 매핑하고 두 곳에서 노출: 기존 `GET /api/v1/members/me/profile` 응답에 **랭킹 요약**(tier·level·score·nextTier·pointsToNext) 추가 → 프로필 탭이 호출 1회로 그려짐, 신규 `GET /api/v1/members/me/ranking` 이 요약 + **breakdown**(reviews·diversity·scans 각 count·points) 제공. 공식 `score = 리뷰 수 × 10 + 고유 음식 수 × 5 + 스캔 수 × 2`, 등급 진입점 newcomer 0 · taster 30 · explorer 80 · regular 180 · gourmet 350 · kfood_master 600 · korean_at_heart 1000(경계값은 상위 등급, 최고 등급이면 nextTier·pointsToNext = null). 서버는 **tier 안정 키만** 내려주고 번역명은 만들지 않는다(FE i18n). 산정 로직은 `:core:member` 의 순수 값 객체 `MemberRanking`+`RankingTier` 로 두고 **평면 카운트 3개(Int)만 입력**받아 scan·review 타입 결합을 피한다(원칙 II). **스캔 횟수 = 메뉴판 1장 1회** — `scan_history` 는 매칭 음식마다 행이 생겨 횟수 집계에 못 쓴다. 그래서 신규 카운터 테이블 **`member_ranking`**(member_id UNIQUE + scan_count, Flyway 1건)을 두고 `ScanUseCase.assessMenuBoard` 가 스캔 1회마다 `MemberRankingRepository.increaseScanCount` 호출(매칭 0건이어도 1회). 카운트업은 `INSERT … ON DUPLICATE KEY UPDATE` 로 원자적(유니크 키가 전제) — 점수·등급은 저장하지 않고 조회 시점 계산. **리뷰 도메인(`:core:review`)은 빈 placeholder 라 리뷰 수·고유 음식 수는 0 고정** — 구현 없는 포트+가짜 어댑터를 만들지 않고 유스케이스가 0을 넣는다(리뷰 기능 도입 시 카운트만 채우면 공식·등급표·응답 계약 무변경). `MemberRankingUseCase` 는 경계 DTO `MemberRankingResult` 를 반환(app:api 가 도메인 타입 미참조), `MemberProfileUseCase.getMyProfile` 이 이를 호출해 요약을 얹는다(회원 SELECT 1회 추가를 감수하고 산정 경로 단일화 → 요약↔상세 불일치 원천 차단). 인증 설정 무변경(`/api/v1/members/*` 는 JWT 필터가 이미 덮음). 테스트: `MemberRankingTest`(공식·경계 전수·최고 등급 null·검증 케이스 128/explorer/52) + `MemberRankingRepositoryAdapterTest`(upsert 카운트업·회원 격리·미기록 0) + `ScanUseCaseHistoryTest`(메뉴판 1장=1회) + `MemberRankingUseCaseTest` + `MemberControllerTest`(프로필 랭킹 요약·상세 breakdown·401·요약↔상세 일치·최고 등급 null). **기존 회원 스캔 횟수는 소급되지 않는다**(배포 이후 스캔부터 누적). 병렬 작업 KB-124(프로필 수정 부분 수정 전환)가 `MemberProfileUseCase`·`MemberController`·`MemberControllerTest` 를 함께 건드리므로 먼저 머지된 쪽 기준 리베이스. FE 공유 필요: 정책 문서는 랭킹을 상세 엔드포인트에서만 받는 전제였다.)

이전 플랜: `specs/kb-124-partial-profile-update/plan.md` (KB-124 프로필 부분 수정 — `PATCH /api/v1/members/me/profile` 을 전체 덮어쓰기 → **진짜 부분 수정**으로. 현재 버그: 닉네임 화면에서 저장하면 `avoidanceSubstanceCodes` 미전송이 기본값 `emptyList()` 로 들어가 **기피 성분이 조용히 전부 삭제**되고, 기피 성분만 보내면 nickname/countryCode/appLanguage 가 non-null 필수라 **400**. R1: `ProfileUpdateRequest` 전 필드를 **nullable + 기본값 null** 로 — 부재→null(유지), `[]`→emptyList(전부 해제)로 **미전송과 빈 배열을 타입 레벨 구분**(명시적 null = 미전송 = 유지, 개별 항목 비우기는 도입 안 함). Optional/JsonNullable 래퍼 기각. R2: **도메인 무변경** — `MemberProfile` 은 private ctor + `@ConsistentCopyVisibility`(private copy) 지만 닉/국가/언어가 이미 nullable 이고 **`of(...)` 가 공개 팩토리**라, 유스케이스에서 `input.x?.let(::validate) ?: current.x` 로 필드별 해소 후 재조립하면 끝(도메인에 merge 팩토리 넣으면 전송계층 개념 유입 → 기각). 맵기 선호도는 지금도 current 에서 옮겨오는 코드 그대로. R3: **입력 타입 분리 필수** — 지금 온보딩·수정이 `MemberProfileInput`(전 필드 non-null) 하나와 `validatedProfile()` 하나를 공유해, 그대로 nullable 로 풀면 **온보딩의 전 필드 필수까지 무너진다**(FR-007). 신규 `ProfileUpdateInput`(전 필드 nullable) + 검증을 필드 단위 4함수(`validatedNickname`·`validatedCodes`·`validatedCountry`·`validatedLanguage`)로 분해해 두 경로 공유. `OnboardingRequest`·`MemberProfileInput` 무변경. R4: 빈 요청 `{}` → 200·무변경(병합 결과가 기존과 동일해 멱등, 별도 분기 없음). R5: 병합 완료 후 **단일 `repository.update`** 라 검증 실패 시 저장에 도달하지 않음 → 부분 저장 구조적 불가. 오류코드는 기존 `OnboardingErrorCode` 4종(400) 재사용. **Flyway·도메인·영속·MemberController 전부 무변경** — 소스 4파일(신규 `ProfileUpdateInput` 1 · 수정 `ProfileUpdateRequest`/`MemberApi`/`MemberProfileUseCase` 3). 테스트: 유스케이스 페이크 단위(미전송 유지/빈배열 해제/닉만/빈요청/전달된 값만 무효→거절+불변/맵기 보존) + MemberControllerTest MockMvc(조회 API 로 관측 — 기피성분은 DB 컬럼 아니라 profile JSON). **회귀 가드: 기존 온보딩 테스트 전부 green 유지.** 헌법 위반 없음(Complexity Tracking 공란).)
<!-- SPECKIT END -->
