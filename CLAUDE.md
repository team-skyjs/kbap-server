...
`specs/kb-191-profile-provider/plan.md` (KB-191 내 프로필 조회 응답에 소셜 로그인 연동 정보(provider) 추가 — `GET /api/v1/members/me/profile` 응답에 `provider` 필드(GOOGLE/APPLE, non-null String) 추가. `Member.provider: SocialProvider`(ENUM, NOT NULL) 저장값을 그대로 노출 — `MyProfileResult.of()` 에 `member.provider.name` 매핑 + `MyProfileResponse.from()` 관통(enum 직노출 대신 String, `countryCode?.name` 선례). 변경 4파일: 프로덕션 2(dto)·Swagger 문서(`MemberApi` 예시 JSON)·테스트(`MemberControllerTest` provider assertion = Red 진입점). 기존 필드 하위 호환 불변, 수정 API 요청 계약 불변(provider 조회 전용). 신규 API·DB 스키마·Flyway·모듈 그래프·신규 파일 0, :app:batch 범위 밖.)

이전 플랜: `specs/kb-189-scan-name-translation/plan.md` (KB-189 스캔 응답 DB 매칭 음식명 번역 버그 수정 — 스캔 응답 `ScanResult.ItemRiskResult.name` 이 항상 비전 추출값(`menu.name`)으로 나가는 버그(`ScanService.kt:55`). DB 매칭·READY 음식은 기존 `Food.displayName(lang)`(nameTranslations, `LocalizedText.resolve` ko 폴백)을 회원 앱 언어로 조립해 `name` 에 담고, 미매칭·INCOMPLETE 는 비전 추출 이름 유지(번역 원천 없음). 회원 언어는 `memberService.getMember(memberId).profile.appLanguage ?: KO`(미설정 → ko 는 헌법 V-1 — Home 의 `?: EN` 은 게스트 전용 관례라 비적용). 번역 조립은 `ScanService.scanMenuBoardImage` 항목 매핑 한 지점(응답 name 의 유일 경로 = 루트 픽스), `recordHistory` 는 `menu.name` 사용이라 이력 저장 자동 불변. **프로덕션 2파일(ScanService 핵심 diff ~3줄 + ScanResponse Swagger 문구 — 현 `name` 설명이 "비전 인식이 읽은 원문"으로 미번역 동작을 명시 중이라 새 의미로 갱신)** — 신규 파일·DTO·API 계약(필드 구조)·DB·Flyway·설정 0, data-model·contracts 없음. Test-First: 기존 `ScanControllerTest`(MockMvc+Testcontainers+FakeVision) 확장 — 시드 헬퍼에 회원 profile(appLanguage)·음식 name_translations 파라미터 추가, 번역 응답(en)·미매칭 유지·ko 폴백 시나리오 Red 후 구현. 기존 매칭 name 단언 `"Kimchi 김치찌개"` → `"김치찌개"` 갱신 필요(appLanguage null 회원 → ko). ScanService 단위 테스트 신설은 기각(의존 5개 페이크 비용, 통합이 동일 경로 검증). :app:batch 범위 밖.)
