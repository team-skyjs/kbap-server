...
`specs/kb-62-menu-search/plan.md` (KB-62 검색어 메뉴 조회 API — 검색어가 **한국어명 + 요청 언어(`lang`) 번역명** 두 이름 중 하나에 포함되는 메뉴를 최신순(id desc) keyset 커서로 20개씩 내려주는 `GET /api/v1/foods/search`. KB-63 목록 조회의 **near-clone** — 페이지네이션·`Page<MenuSummaryResponse>`·`BaseResponse`·언어폴백(원칙 V)·회피 위험도·2단계 페이징(id 조회→id-in fetch join)을 전부 재사용하고 **매칭 술어 하나만** 다름. 번역명은 `name_translations` JSON 컬럼 저장이라 요청 언어 매칭은 `JSON_EXTRACT(name_translations,'$."<lang>"') LIKE '%kw%'` — JPQL 표준화 어려워 **네이티브 쿼리**로 작성하고 그 대가로 `@SQLRestriction` 자동 ACTIVE 필터가 빠지므로 `status='ACTIVE'` 명시(회귀 테스트 필수). `lang=KO`(미지정 폴백)면 한국어명만, 그 외면 한국어명 OR 해당 언어 번역명. 대소문자 비구분은 MySQL `utf8mb4_0900_ai_ci` 콜레이션이 처리(LOWER 불요). leading-wildcard LIKE 풀스캔 수용 — **신규 인덱스·마이그레이션 없음**(카탈로그 규모 기준, 커지면 FULLTEXT+ngram 승격). 빈/공백 검색어는 `BLANK_SEARCH_KEYWORD` 400(검색어 없는 전체 탐색은 KB-63 목록 API 담당 — 역할 분리). 신규 파일 ~7개(port 메서드·네이티브 쿼리·어댑터·`SearchMenusInput`·키워드 resolver·`SearchMenusUseCase`·컨트롤러+스웨거), 응답 DTO/봉투/`BrowseMenusResult`/`MenuSummaryView`/언어·커서·회피 컴포넌트는 재사용. browse/search 위험도·뷰 조립 중복은 `MenuSummaryAssembler` 로 추출 권장. 테스트: 유스케이스 단위(페이크 포트) + Testcontainers 어댑터(실 JSON 매칭·언어분리·대소문자·소프트삭제제외) + MockMvc 통합.)
<!-- SPECKIT END -->
