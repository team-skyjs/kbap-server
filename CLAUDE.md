...
`specs/kb-49-llm-client-foundation/plan.md` (KB-49 LLM 호출 토대. 신규 **LLM 전용 모듈 `:infra:llm`**(내부에서 Spring AI `ChatModel` 의존 — 초안 `:infra:client`·예고 `:infra:external` 대체, ADR-0010)에 공개 API `LlmFanoutClient`(generate→LlmFanoutResult)·값타입·구성 전부. **`:core:kernel` port·runtimeOnly 조립은 생략** — `:app:batch` 가 `:infra:llm` 를 **`implementation` 으로 직접 의존**해 잡에서 호출(단일 소비자=배치, 속도 우선; web/application 재사용 시 kernel port 승격 후속). OpenAI·Upstage(OpenAI 호환 base-url 교체)·Gemini(google-genai) 3개 `ChatModel` 을 `meogo.llm.*` 프로퍼티+`@ConditionalOnProperty` 로 명시 구성 — 키 없으면 빈 미생성 → batch/web 부팅 안전(`spring.ai.model.*=none`). fan-out 은 **JDK21 가상스레드+CompletableFuture**, 단일모델 seam `LlmModelCaller`로 부분실패를 페이크 단위검증(헌법 I). 배치는 단일 `:app:batch` bootJar(잡을 user/admin 패키지 구분). 영속/DB 무변경. 문서 갱신은 현재 구현 직결분만 — 범용 `:infra:external` catch-all 서술은 범위 밖(2026-07-06 clarify)).
<!-- SPECKIT END -->
