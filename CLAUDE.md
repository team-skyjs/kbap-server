...
이전 플랜: `specs/kb-138-menu-photo-scan/plan.md` (KB-138 메뉴판 사진 스캔 — 업로드 완료 검증 + gpt-4o-mini 메뉴명·가격 추출. 두 API: (1) `POST /api/v1/images/complete` — presigned 업로드 후 완료 신고, HeadObject 로 실제 Content-Type(`image/*`)·크기 대조, 실패 시 DeleteObject + `IMAGE-001~003`, 성공 시 `uploaded_image` 기록(멱등). (2) `POST /api/v1/scans` — `{imagePath}` 검증·본인 소유 확인(`SCAN-001`) 후 vision 추출 → food 매칭·위험도 판정, 응답 additive(항목별 `price`, vision 실패 `SCAN-002` 503). 신규 모듈: **`:domain:image`**(`UploadedImage` 엔티티·`ImageUploadService`·`StorageObjectStore` seam) + **`:infra:storage`**(S3 head/delete, KB-145 presign 도 이 모듈). vision 은 `:core` seam `MenuBoardVisionExtractor` + `:infra:llm` `OpenAiMenuBoardVisionExtractor`(`kbap.llm.vision.*`). `scan_history` 확장(image_path·menu_name·price, food_id nullable). Flyway 2건. 외부 호출은 트랜잭션 밖.)
