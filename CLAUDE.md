...
`specs/kb-90-menu-name-refinement/plan.md` (KB-90 메뉴 스캔 수신 메뉴명 정제 — 흐름: **정규화 → 전부 LLM 음식명 추출 → DB 매칭 → hit/miss → miss 대기열**(사용자 결정). (1) `:core:kernel` `KoreanMenuNameNormalizer.matchKey`(NFC 후 `[가-힣]`만) — 빈 키(한글 0자)는 LLM 스킵하고 NOT_FOOD 게이트. (2) 비지 않은 전 항목을 `:core:kernel` port `ScannedNameInterpreter`(배열→ StandardName|NotFood)로 Upstage LLM 1콜(`:infra:llm` 단일 caller 어댑터, `@ConditionalOnProperty`, `:app:api` runtimeOnly 조립). (3) 표준명을 `matchKey` 로 `FoodRepository.findByKoreanMatchKey` exact 매치 — hit=MATCHED/miss=PENDING+`pending_menus`(unique dedup)/NotFood=제외. foods 에 MySQL 생성 저장 컬럼 `korean_match_key`(`REGEXP_REPLACE(korean_name,'[^가-힣]','')`)+인덱스(Flyway, 백필 불요), kernel↔SQL 동등성 sync 테스트로 드리프트 차단. **폴백**: LLM 미구성·실패·타임아웃 시 정규화 exact 매치로 강등 — 이름 정확한 아는 메뉴는 MATCHED 유지, 나머지 원문 PENDING+대기열, 스캔은 항상 성공(트랜잭션 밖 호출). 정규화 "깨끗함"은 형태로 판정 불가 → 판정자는 DB exact 매치 또는 LLM. `ScannedMenuItem` 에 매칭 상태(MATCHED foodId/PENDING/NOT_FOOD, 3종 종결) 추가, 응답에 `matchStatus`·`foodId` 미러링(하위 호환, 요청 스키마 불변). 캐시 없음. 위험도 산출은 mock 유지(범위 밖). 구현 순서 P1(정상 경로 전체 MVP)→P2(폴백). 동음이의 food 는 최소 id 매칭+로깅(위험도 de-mock 시 재검토)).
<!-- SPECKIT END -->
