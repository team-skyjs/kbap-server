...
`specs/kb-90-menu-name-refinement/plan.md` (KB-90 메뉴 스캔 정제·매칭·위험도 — `POST /api/v1/scans`. 흐름: **정규화(NFC→한글만, 빈 키=메뉴 아님) → 비지 않은 전 항목을 Upstage `solar-pro` 동기 1콜(같은 길이 배열 + `NOT_FOOD` 센티넬, temperature 0, 5초 타임아웃·재시도 없음) → `findByKoreanMatchKeys` 배치 1쿼리 매칭 → 위험도 산출**. miss 는 별도 대기열 없이 **`food` 에 `content_status=INCOMPLETE` 로 in-place 등록**(다중행 `ON DUPLICATE KEY UPDATE` upsert — `saveAll` 은 경합 데드락·유령행 롤백으로 깨진다)하고, 완성(READY) 전까지 목록·검색·상세에 노출 안 함(serving gate 는 목록 JPQL·검색 네이티브 쿼리 + 상세 어댑터에만 — 스코어링 배치가 쓰는 공유 쿼리엔 미적용, 안 그러면 미완성 음식이 영영 안 채워짐). **`Food.overallRisk()` 가 `!isReady()` 면 `UNKNOWN` 강제** — `RiskLevel.aggregate(빈 목록)=SAFE` 함정을 도메인에서 차단. 위험도는 유스케이스가 `Food.overallRisk(회피코드)` 직접 호출(더미 회피성분 제공자, 카탈로그 미조회 — KB-62 와 동일 규칙). **폴백**(LLM 호출 예외·타임아웃·응답 개수 불일치): 정규화 exact 매치만 하고 **미완성 food 를 만들지 않으며**(테이블 오염 방지) 응답에 `degraded=true`. `UPSTAGE_ENABLED` 기본 on — 키가 없으면 부팅 실패(정제 없이 스캔 API 를 열지 않는다). **스캔 내역 미저장**(`menu_scan`·`scanned_menu_item` DROP, `:core:scan` 모듈까지 삭제 — 스캔은 상태가 없다), **요청에서 바운딩박스·`lang` 미수신**(표시명은 한국어 고정, 회원 언어 설정 연동은 TODO). 응답 = `{degraded, results[]{idx, matched, foodId?, riskLevel, name?, koreanName?}}` — **비음식은 결과에서 제외**되어 개수가 요청보다 적을 수 있고 클라이언트는 `idx` 로 짝을 맞춘다(`foodId` 가 있어도 `matched=false` 일 수 있다 — 조사 대기 음식). Flyway `food.korean_match_key` 생성 컬럼은 **`COLLATE utf8mb4_bin` 필수**(기본 collation 에선 `[^가-힣]` 범위 오류로 마이그레이션 실패 — Testcontainers 가 못 잡음). 상태 컬럼 8종은 `ENUM` — 엔티티에도 `columnDefinition`/`uniqueConstraints` 를 선언해야 테스트 스키마가 프로덕션과 일치한다.)
<!-- SPECKIT END -->
