...
`specs/kb-104-onboarding-profile/plan.md` (KB-104 온보딩 — `POST /api/v1/members/me/onboarding`(닉네임·기피 성분 코드 목록·국가·앱 언어 제출 → 프로필 저장 + `onboardingCompleted` false→true 전이, 재제출 400)·`GET /api/v1/members/me`(홈화면 분기용 프로필+상태 조회). 도메인·영속은 `Member.updateProfile`/`completeOnboarding`·`MemberProfileJson`(JSON 칼럼) 재사용(KB-103/117)하되 **온보딩 표현을 전 계층 `onboardingCompleted` boolean 으로 통일**(사용자 결정, research R8): 도메인 enum `OnboardingStatus` 삭제·boolean 프로퍼티화, 엔티티 enum↔boolean 변환 제거, **Flyway 1건** `onboarding_status`→`onboarding_completed` 칼럼 rename(기존 적용 파일 무수정, 통합 테스트가 Testcontainers Flyway+`ddl-auto=validate` 로 자동 검증). 신규 3종: (1) **요청 인증 최초 도입 = 필터 레벨(사용자 결정, R1 개정)** — app:api `common/auth` 에 `JwtAuthenticationFilter`(OncePerRequestFilter, `FilterRegistrationBean` urlPatterns `/api/v1/members/*`, `TokenParser.parseAccessToken`(PR #46: memberId+role) 검증→request attribute 저장, 실패는 필터가 직접 401 BaseResponse JSON — advice 미도달·헤더 부재·위조=INVALID_ACCESS_TOKEN·만료=EXPIRED_ACCESS_TOKEN) + `@AuthMemberId` resolver 는 attribute 주입만. 전 API 일괄 인증은 추후 urlPatterns 확장. Spring Security 미도입. (2) application:client `member` 패키지 — `CompleteOnboardingUseCase`(단일 @Transactional: 조회→검증→updateProfile→completeOnboarding→save 1회, 저장 전 검증으로 실패 시 무변경)·`GetMyProfileUseCase`·`OnboardingErrorCode`(400: INVALID_NICKNAME/INVALID_AVOIDANCE_SUBSTANCE_CODE/INVALID_COUNTRY_CODE/UNSUPPORTED_APP_LANGUAGE, ~습니다 체). **카탈로그 81종 멤버십 검증은 application 에서 `AvoidanceSubstanceCode` enum 대조**(헌법 II — member 는 코드 문자열만, DB 조회 0)·국가=`CountryCode.from`·언어=`LanguageCode.code` 정확 일치(불일치 400 fail-fast)·닉네임 trim 후 비어있지 않음. (3) app:api `member` 패키지 — MemberApi/MemberController(auth 슬라이스 미러링). 맵기 선호도는 입력 제외(기본 5 유지, 후속 프로필 수정에서). 기피 성분 빈 배열 허용·중복은 Set 화. 테스트: 유스케이스 = 페이크 MemberRepository 단위, 컨트롤러 = MockMvc+Testcontainers 통합(유효/무효/상태전이/401 — DoD). 헌법 위반 없음(Complexity Tracking 공란).)
<!-- SPECKIT END -->
