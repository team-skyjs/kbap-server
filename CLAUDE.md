...
`specs/kb-123-member-ranking/plan.md` (KB-123 회원 랭킹 — 활동량을 점수로 환산해 7단계 등급으로 매핑하고 두 곳에서 노출: 기존 `GET /api/v1/members/me/profile` 응답에 **랭킹 요약**(tier·level·score·nextTier·pointsToNext) 추가 → 프로필 탭이 호출 1회로 그려짐, 신규 `GET /api/v1/members/me/ranking` 이 요약 + **breakdown**(reviews·diversity·scans 각 count·points) 제공. 공식 `score = 리뷰 수 × 10 + 고유 음식 수 × 5 + 스캔 수 × 2`, 등급 진입점 newcomer 0 · taster 30 · explorer 80 · regular 180 · gourmet 350 · kfood_master 600 · korean_at_heart 1000(경계값은 상위 등급, 최고 등급이면 nextTier·pointsToNext = null). 서버는 **tier 안정 키만** 내려주고 번역명은 만들지 않는다(FE i18n). **랭킹은 `Member` 애그리거트의 하위 개념** — 별도 카운터 테이블·리포지토리를 두지 않고 `member.scan_count` 컬럼(Flyway 1건, `DEFAULT 0`)에 카운트를 두고 `Member.signUp` 이 0으로 초기화, `Member.recordScan()`(불변) 이 올리고, `Member.ranking()` 이 자기 카운트에서 `MemberRanking`(`:core:member` 순수 값 객체 + `RankingTier`)을 파생한다. 탈퇴 시 회원과 함께 사라지고 가입 초기화도 공짜다. **스캔 횟수 = 메뉴판 1장 1회** — `scan_history` 는 매칭 음식마다 행이 생겨 횟수 집계에 못 쓴다. `ScanUseCase.assessMenuBoard` 가 스캔마다 회원 로드 → `recordScan()` → `MemberRepository.update`(매칭 0건이어도 1회). **동시성은 의도적으로 감수**(read-modify-write → 동시 스캔 시 1회 유실 가능; 초기 단계·비핵심 영역이라 락/버전/원자적 upsert 는 과설계. 문제 시 이벤트 기반 집계로 전환 — 응답 계약 불변). 점수·등급은 저장하지 않고 조회 시점 계산이라 배점·등급표가 바뀌어도 재계산 배치가 없다. **리뷰 도메인(`:core:review`)은 빈 placeholder 라 리뷰 수·고유 음식 수는 0 고정**(`Member.ranking()` 이 0 주입 — 리뷰 기능 시 같은 방식으로 member 에 카운트 추가, 공식·등급표·응답 계약 무변경). `MemberRankingUseCase` 는 경계 DTO `MemberRankingResult` 반환(app:api 가 도메인 타입 미참조), `MemberProfileUseCase.getMyProfile` 이 이를 호출해 요약을 얹는다(산정 경로 단일화 → 요약↔상세 불일치 원천 차단). 인증 설정 무변경(`/api/v1/members/*` 는 JWT 필터가 이미 덮음). 신규 리포지토리·엔티티 없음 — `MemberJpaEntity.applyDomain` 이 scan_count 왕복까지 처리. 테스트: `MemberRankingTest`(공식·경계 전수·최고 등급 null·검증 케이스 128/explorer/52) + `MemberTest`(가입 시 0·recordScan 불변·프로필 갱신 시 보존) + `MemberRepositoryAdapterTest`(scan_count 영속) + `ScanUseCaseHistoryTest`(메뉴판 1장=1회) + `MemberRankingUseCaseTest` + `MemberControllerTest`(가입 직후 0점·프로필 랭킹 요약·상세 breakdown·401·요약↔상세 일치·최고 등급 null). **기존 회원 스캔 횟수는 소급되지 않는다**(배포 이후 누적). 병렬 작업 KB-124(프로필 부분 수정)가 `MemberProfileUseCase`·`MemberController`·`MemberControllerTest` 를 함께 건드리므로 먼저 머지된 쪽 기준 리베이스. FE 공유 필요: 정책 문서는 랭킹을 상세 엔드포인트에서만 받는 전제였다.)
