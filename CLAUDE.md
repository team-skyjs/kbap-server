...
`specs/kb-99-always-korean-menu-name/plan.md` (KB-99 언어 무관 메뉴명 한국어 항상 포함 — 상세·목록 응답에 지역화 메뉴명과 **별개 필드 `koreanName`**(언어 무관 한국어 원문)을 추가. 지역화명이 곧 한국어인 경우(`lang=ko` 또는 번역 부재 ko 폴백)에는 **`koreanName=null`**(중복 미노출, 사용자 결정). **DB·영속·Flyway 변경 없음** — 한국어 원문은 기존 `LocalizedText.korean`(`foods.korean_name` NOT NULL)에서 이미 로드됨. 3계층 수직 슬라이스: (1) core:food 에 `Food.koreanName()`/`FoodContent.koreanName()` seam, (2) application:client 유스케이스가 `koreanName = food.koreanName().takeIf { it != 지역화명 }` 계산해 `GetFoodDetailResult`·`BrowseMenusResult.MenuSummaryView` 에 `koreanName: String?` 담음, (3) app:api `FoodDetailResponse`·`MenuSummaryResponse` 에 필드 미러링(+`@Schema`). 순수 필드 추가 = 하위 호환, 요청 파라미터·기존 지역화·폴백 값 불변(회귀 없음). **범위 검토(사용자 요청): 추가 수정 API 없음** — 스캔 API 는 메뉴명을 지역화·반환하지 않고 클라이언트 `rawMenuName` 을 받으므로 범위 밖. 검증: en/ja/ko·미지원 폴백 BehaviorSpec 통합 테스트(SC-003)).
<!-- SPECKIT END -->
