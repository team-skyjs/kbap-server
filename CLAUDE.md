...
`specs/kb-147-profile-image-url/plan.md` (KB-147 프로필 사진 URL·맵기 선호 필드 추가 — 온보딩 요청·프로필 부분 수정 요청·내 프로필 조회 응답에 `profileImageUrl` 과 `spicinessPreference`(기존 도메인 속성 0~10·기본 5, 현재 입출력이 막혀 있어 개방 — DTO 둘 다 `Int? = null` 선택 입력, 미전송=기존 값 유지, 범위 밖은 `validatedSpiciness` 가 신규 `INVALID_SPICINESS_PREFERENCE`("MEMBER-009", 400) 로 선행 거절, `MemberProfile` init require 는 최후 방어선) 추가. **저장은 기존 `member.profile` JSON 컬럼의 `MemberProfileJson` 필드 추가 — Flyway 마이그레이션 0건**(기존 row 는 키 부재 → Jackson 기본 null = 미설정, Jira DoD 의 "nullable 컬럼" 의도 충족). 부분 수정 3분법: 미전송(null)=유지 · 값=검증 후 교체 · **빈 문자열=제거**(별도 삭제 API 없음, 사용자 확정). 검증은 `MemberService` 의 기존 `validated~` 관례로 `validatedImageUrl` — https 스킴 + URI 파싱 + 호스트 존재 + 길이 ≤512 + **허용 호스트 목록 `kbap.member.profile-image-allowed-hosts`**(콤마 구분, `@Value` 생성자 주입, 빈 값(기본·local·테스트)이면 형식 검증만, prod 는 CloudFront CDN 호스트 등록 — 사용자 확정) 불합격 시 신규 `ErrorCode.INVALID_PROFILE_IMAGE_URL`("MEMBER-008", 400). 변경 모듈 3개(:core ErrorCode·:domain:member·:app:api), 신규 파일 0건 — MemberProfile/MemberProfileJson/MemberProfileInput/ProfileUpdateInput/MyProfileResult + OnboardingRequest/ProfileUpdateRequest/MyProfileResponse/MemberApi 에 필드 추가, 신규 엔드포인트·DB 스키마 변경 없음. 테스트: MemberServiceTest(도메인 통합, 온보딩 등록/미등록·수정 교체/유지/제거·검증 거절) + MemberControllerTest(MockMvc) — Kotest BehaviorSpec 한국어 given/when/then, Red→Green 테스트 우선. presigned URL 발급·이미지 업로드는 별도 태스크로 범위 밖.)

이전 플랜: `specs/kb-138-menu-price-mapping/plan.md` (KB-138 메뉴판 사진 → 메뉴명·가격 추출 퀄리티 실험 — **스파이크, 프로덕션 코드 무변경**. 메뉴판 사진의 **이미지 URL** 을 GPT vision(OpenAI chat API)에 직접 전달(OpenAI 가 URL fetch — 이미지 바이트는 하네스/서버 미통과, 실환경 전제 = 클라이언트가 S3 presigned URL 업로드 후 링크만 백엔드 전달)해 메뉴명·가격 쌍을 구조화 JSON 으로 추출하고, 수기 정답 라벨 대비 지표 4종(메뉴명 정확도(누락 MISSING·오검출 SPURIOUS·오타 구분), 가격 정확도, 지연, 장당 토큰·비용)을 측정한다. **하네스 = `:infra:llm` 테스트 소스셋** `com.kbap.infra.llm.experiment` — `LlmSmokeTest` 의 `@EnabledIf` + 시스템 프로퍼티(`llm.vision.experiment.enabled=true`) opt-in 패턴 재사용(CI skip 안전), Spring AI `OpenAiChatModel` 직접 생성 + `UserMessage` 에 URL `Media` 첨부(`LlmFanoutClient`/`LlmChatRequest` 는 텍스트 전용이라 미사용 — 확장은 후속 이슈). 순수 로직 `MenuPriceParser`(코드펜스 제거 + Jackson, `ScannedNameParser` 관례)·`ExperimentMetrics`(정규화 정확 일치 매칭)는 Red→Green TDD. 기본 모델 `gpt-4o-mini`, `-Dllm.vision.experiment.model` 로 교체. 실험 자산은 `specs/kb-138-menu-price-mapping/experiment/` — `samples.json`(URL·조건 태그·정답 라벨 인라인), `results.json`(summary+샘플별 결과, 실패 URL 은 error 원인 기록·전체 중단 금지), `report.md`(현행 클라이언트 OCR+Upstage 텍스트 정제 대비 비교표, 채택/미채택 결론, 후속 이슈: LlmChatRequest 이미지 입력 확장·가격 스키마·ScannedNameInterpreter seam 재설계·Gemini 는 URL fetch 미지원 유의). DB·엔티티·마이그레이션·API 0건.)
