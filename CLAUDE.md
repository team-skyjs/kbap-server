...
`specs/kb-98-food-detail-by-food-id/plan.md` (KB-98 음식 상세 조회 foodId 정합 — 조회 진입점만 한국어 메뉴명 → 안정적 숫자 식별자 **foodId** 로 교체. 상세 산출 로직(언어 해석·번역 폴백·성분/위험도·응답 스키마)은 전부 재사용. 엔드포인트 `GET /api/v1/foods/{foodId}?lang=`(기존 `/foods/detail?menuName=` 제거). 미존재·소프트삭제·비숫자 foodId 는 모두 **400** — `FoodErrorCode.NOT_FOUND` 가 이미 status=400, 소프트삭제는 `BaseEntity @SQLRestriction` 자동 제외 → null → 400, 비숫자는 `GlobalExceptionHandler` 에 `MethodArgumentTypeMismatchException` 핸들러 1개 추가. **최소 변경**: `FoodRepository.findById(id)` port 1개 + 어댑터가 기존 fetch-join `findByIdInWithAvoidanceSubstances(listOf(id)).firstOrNull()` 재사용(새 JPA 쿼리·마이그레이션 없음) + `GetFoodDetailInput.menuName→foodId` + usecase 한 줄 + controller `@PathVariable`. 상세 usecase 에서 `findByKoreanName` 호출 제거. 스키마 불변(SC-003). 헌법 5원칙 위반 없음).
<!-- SPECKIT END -->
