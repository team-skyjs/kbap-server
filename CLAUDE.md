...
`specs/kb-46-mysql-testcontainers/plan.md` (DB-backed 통합 테스트를 H2→**MySQL 8.4 Testcontainers**로 교체. **운영 코드·엔티티·마이그레이션 SQL 무변경 — 테스트 실행 DB 환경만 교체.** P1(핵심): `app:api` 테스트에서 **Flyway ON + `ddl-auto=validate`** 로 전환해 지금껏 테스트에서 한 줄도 안 돌던 운영 마이그레이션 전체(MySQL 전용 `JSON_OBJECTAGG`·`MODIFY COLUMN` 등)를 실 엔진에서 검증(현재 `flyway.enabled=false`+`create-drop` 사각지대). P2: `infra:persistence` 어댑터 테스트도 실 MySQL 엔진에서 실행(마이그레이션 없어 스키마는 Hibernate 생성 유지). P3: `@ServiceConnection`+Spring 컨텍스트 캐싱으로 컨테이너를 **모듈 테스트 JVM당 1회** 재사용. 공유 설정은 `infra:persistence` **testFixtures**(`MySqlContainerConfig`/`MySqlIntegrationSpec`)에 단일화하고 `app:api`가 `testImplementation(testFixtures(...))`로 소비(순환 없음). H2 제거(FR-008). 리스크: 마이그레이션 첫 실전 실행·`validate` 드리프트·시드 픽스처↔Flyway 시드 정합(D-7). 헌법 "MySQL(+H2 test)" 문구는 구현 후 PATCH 후속).
<!-- SPECKIT END -->
