...
`specs/kb-111-home-screen/plan.md` (KB-111 홈 화면 조회 — `GET /api/v1/home` 단일 호출로 3섹션(회원 기피 성분·인기 음식 5개·최근 스캔 10개)을 한 응답으로. **선택적 인증 최초 도입**(research R1): app:api `common/auth` 에 `@AuthMemberIdOrNull` + `AuthMemberIdOrNullArgumentResolver` — 헤더 없음/`Bearer ` 아님→null(비회원), 헤더 있으면 `TokenParser.parseAccessToken` 검증(위조·만료→`AuthException`→GlobalExceptionHandler 401). 강제 필터(`/members/*`)는 무수정. 신규 컨텍스트 **`:core:scan`**(헌법 II 예약 자리) — `ScanHistory`@AggregateRoot(memberId·foodId·scannedAt=BaseEntity createdAt 재사용, `record()` 팩토리) + `ScanHistoryRepository` port(saveAll·`findRecentReadyFoodIds(memberId,limit)`). 영속 신규 `scan_history` 테이블(FK 없이 ID 참조, `idx_scan_history_recent(member_id,created_at)`) + Flyway 1건(timestamp 버전). `findRecentReadyFoodIds` 네이티브: `JOIN food` READY·ACTIVE + `GROUP BY food_id`·`MAX(created_at) DESC`·limit 로 dedup·정렬·완성필터 일괄. `ScanUseCase` 는 memberId!=null & 매칭 READY 음식만 이력 기록(외부 호출 뒤 단발 write). **`MockAvoidedSubstanceProvider` 삭제→`MemberAvoidedSubstanceProvider`**(research R2): `avoidedCodes(memberId: Long?)` 로 시그니처 변경(무인자→memberId), 비회원 emptySet·회원 프로필 코드→`AvoidanceSubstanceCode`. 소비자 4개(Browse/Search/GetFoodDetail/Scan UseCase) Input 에 memberId:Long? 추가·컨트롤러 `@AuthMemberIdOrNull` 배선 → 음식 목록/검색/상세/스캔이 선택 인증 개인화(기존 통합 테스트는 회원+토큰 시드 또는 비회원 empty 기대로 갱신 필요). 인기 음식 = `FoodRepository.findRandomReady(size)` 네이티브 `ORDER BY RAND()`(지표 부재 임시, 계약 `FoodSummaryView` 재사용 고정 — 지표 도입 시 정렬만 교체). 언어 = 회원 `appLanguage` ?: `EN`(비회원·온보딩미완료 영어). 기피 성분 섹션 = `AvoidedSubstanceView(code,name)` — `AvoidanceSubstanceRepository.findByCodes`+`displayName(lang)` 지역화. 비회원 개인화섹션 null, 회원 없음 []. 테스트: 유스케이스 페이크 단위 + 컨트롤러 MockMvc+Testcontainers(회원/비회원/무효토큰401/기피없음/스캔없음/dedup). 헌법 위반 없음(Complexity Tracking 공란).)
<!-- SPECKIT END -->
