...
`specs/kb-96-food-candidate-pipeline/plan.md` (KB-96 음식 candidate 스테이징 파이프라인 골격 + 승격 배치 — ADR-0012/0013. 미완성 음식(성분 매핑·설명 번역이 서로 다른 배치·시점에 부분 완성)을 **`food_candidate` 스테이징 테이블**(`:core:research` 소유 도메인 `FoodCandidate` + `:infra:persistence` JPA 어댑터)에 쌓고, 완성 조건(성분 매핑 有 && ko 설명 有 && 9언어 번역 완비 && 미승격)을 만족한 것만 **승격 배치**(`:app:batch` 독립 `ApplicationRunner`, `meogo.promotion.runner.enabled`)가 `food`(+`food_avoidance_substance`)로 **korean_name 업서트 + published_food_id 멱등** 적재(음식 1건=트랜잭션 1개, 부분 실패 격리). `food` 엔 완성본만 존재 → API 서빙 깨끗. **영속은 JPA-first** — 대량 컬럼-스코프 hot-path 는 `@Modifying` 벌크로 persistence context 우회, IO 규칙(컬럼-스코프 부분 업데이트로 동시 실행에도 타 잡 컬럼 보존·정합성 타이밍 비의존, 청크당 라운드트립 최소 `IN`/JDBC 배치 `rewriteBatchedStatements=true`, 멱등). Exposed 는 포트 seam 뒤 후속(규모/컴파일타임 쿼리안전 정당화 시). enrichment(성분 KB-54·번역 KB-94)는 **픽스처로 대체**해 골격 e2e 검증(MySQL Testcontainers, TDD). 컨텍스트 경계는 승격 시 스냅샷 값으로만 이관(도메인 직접 공유 X). `FoodCandidateRepository`(create/findPromotable/updateSubstanceMapping/updateDescriptionTranslations/markPublished)·`FoodRepository.save` 포트. 스코어링→candidate(KB-54)·설명→candidate(KB-94)·food 서빙 읽기(기존 JPA)는 범위 밖).
<!-- SPECKIT END -->
