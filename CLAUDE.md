...
`specs/kb-118-firebase-jwt-auth/plan.md` (KB-118 Firebase 소셜 로그인 — `POST /api/v1/auth/{login,refresh,logout}`. Firebase ID 토큰을 firebase-admin `verifyIdToken` 으로 서명·iss·aud·exp 전부 검증(로그인 시 1회만), 클레임 매핑: sign_in_provider→SocialProvider, **`firebase.identities` 원본 sub → provider_uid(최상위 sub=Firebase uid 저장 금지)**, email(null·애플 릴레이 허용 — 신원 키 아님). 신원 해소(조회→없으면 가입→유니크 충돌 시 재조회 폴백)는 `LoginUseCase` 가 직접 수행 — 도메인 `MemberIdentityResolver` 는 소비자가 로그인뿐이라 유스케이스로 인라인 후 삭제(수동 @Bean 등록 제거, 사용자 결정). **자체 JWT 2종을 응답 본문으로**(쿠키 아님 — 모바일 앱은 쿠키 자동 전송이 없어 개정, 클라이언트가 `Authorization: Bearer` 로 제시): access(30m, stateless, 사용자 클레임 = 회원 PK `sub` 단 하나 — 개인정보 금지 KB-102)·refresh(14d, `jti` UUID). **refresh 저장 = Redis** `auth:refresh:{jti}`→memberId TTL(만료 잔존 없음, 다중 기기 = jti 다중, 로그아웃 = jti 삭제). **재발급 = rotation**: 둘 다 갱신·구 jti 삭제·refresh 유효기간 연장(구 refresh 재사용 = 401). **refresh 만료 = EXPIRED_REFRESH_TOKEN 401 + 강제 로그아웃**(Redis 삭제, 클라이언트는 저장 토큰 삭제 후 재로그인) — 조작·위조·폐기는 INVALID_REFRESH_TOKEN 401. **모듈 배치(사용자 결정·속도 우선): firebase-admin+jjwt+auth 유스케이스 = `:application:client` auth 패키지 — 헌법 III 의식적 위반, plan Complexity Tracking 에 정당화**(infra:firebase 미신설 — 이슈 DoD 대체). `RefreshTokenStore` port 는 core:member, Redis 어댑터·spring-data-redis 는 infra:persistence(기존 패턴). Redis 인프라 신규: docker-compose `meogo-redis`·프로필 설정·Testcontainers. Flyway 없음. 정지 회원 로그인 = 기존 유니크 충돌 메커니즘이 차단(신규 코드 0, `SUSPENDED_MEMBER` 에러는 후속). `SocialTokenVerifier` seam 필수(페이크 단위 테스트 — DoD). 서비스 계정 키·JWT_SECRET 커밋 금지(주입 경로 = quickstart.md).)
<!-- SPECKIT END -->
