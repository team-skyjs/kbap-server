...
`specs/kb-119-member-withdrawal/plan.md` (KB-119 회원 탈퇴 — `PATCH /api/v1/auth/withdraw`(인증 API 그룹 — `AuthApi`/`AuthController`), **요청 본문 없음**(access token 이 대상 회원 결정, `@AuthMemberId`). `/auth/*` 는 로그인·재발급이 공개라 필터 밖이므로 `WebMvcAuthConfig` 에 **`/api/v1/auth/withdraw` 정확 경로만** 추가(와일드카드 금지 — 로그인이 막힌다). 흐름: `findById` 400(회원 미존재 — 프로젝트 유일 404 였던 `MEMBER_NOT_FOUND` 를 400 으로 전환) → 회원의 `(provider, providerUserId)` 로 **서버가 Firebase 역조회** `getUserByProviderUid("google.com"|"apple.com", sub).uid` → `deleteUser(uid)` → `memberRepository.withdraw(id)`. **재인증 없음** — 앱이 탈퇴 직전 소셜 재로그인을 하지 않기로 해 티켓의 "신선한 ID 토큰 첨부" 안을 폐기(대가: access token 탈취 시 계정 삭제 가능, TTL 30분). 그래서 신원 대조·403·`SOCIAL_IDENTITY_MISMATCH`·US3 도 함께 폐기. **Firebase 삭제를 먼저** 하고 실패 시 DB 무변경 + 500(`SOCIAL_ACCOUNT_DELETE_FAILED`) + ERROR 로그(memberId·provider·providerUserId) 남겨 관리자 콘솔 수동 삭제(더미 치환 뒤 실패하면 조회 키인 provider_uid 를 잃음). `USER_NOT_FOUND` 는 성공 흡수(멱등). **`@Transactional` 없음**(외부 호출을 tx 밖에 — 헌법 제약, DB 쓰기는 단발). 신규 port `SocialAccountDeleter(delete(provider, providerUserId))` + `FirebaseAccountDeleter`(둘 다 `:application:client` — KB-118 firebase-admin 완화 승계, Complexity Tracking 1건), `UnavailableSocialTokenVerifier`→`UnavailableSocialAuth`(두 port 구현). 탈퇴 회원 refresh token 무효화는 **Redis 색인 신설 없이** `RefreshUseCase` 가 `consume(jti)` 후 `memberRepository.findById` 존재 확인 → 모든 기기 잔여 토큰 일괄 사망(잔여 키는 TTL 정리). **영속·도메인·Flyway 전부 무변경** — `MemberRepository.withdraw` port·adapter·`MemberJpaEntity.withdraw()`(`provider_uid`→`DELETED:{id}`, status→DELETED)는 KB-117 에 이미 있고 호출자만 없었다. **이메일·닉네임·프로필은 우리 DB 에 유지**(Firebase 쪽 이메일만 계정 삭제로 소멸). `provider_uid` 더미 치환은 최적화가 아니라 **필수** — 소프트 삭제라 유니크 인덱스 `uk_member_provider_uid` 에 항목이 남아 치환 없이는 같은 소셜 계정 재가입 INSERT 가 409. 테스트: `WithdrawUseCaseTest`(페이크 3건 — 삭제 먼저·withdraw 나중, 회원없음 400, 삭제실패 500 + withdraw 미호출) + MemberControllerTest(22)·AuthControllerTest(15) MockMvc+Testcontainers(탈퇴→프로필400·재탈퇴400·미인증401·삭제실패500·refresh401·재로그인 newMember=true·가입탈퇴 2회 반복). `FakeSocialAccountDeleter`(기록+fail 스위치)를 `FakeSocialTokenVerifierConfig` 에 추가. Apple 토큰 revoke 는 범위 밖(조사 후 별도 Jira).)
<!-- SPECKIT END -->
