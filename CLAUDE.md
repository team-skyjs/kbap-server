...
**2026-07-28 모듈 다이어트(KB-244, ADR-0016) + 2026-08-14 infra 해체(ADR-0018)** 이후 모듈은 `:common`·`:api`·`:batch` **3개뿐**이다. 모듈 디렉터리는 **루트 직속**(`common/`·`api/`·`batch/`)이고 껍데기 컨테이너를 두지 않는다. **패키지는 모듈명을 그대로 미러링**한다(`:api`→`com.kbap.api`, `:batch`→`com.kbap.batch`, `:common`→`com.kbap.common`). API 모듈은 ADR-0017에 따라 `com.kbap.api.<feature>` 기능 패키지로 평탄화하며 `com.kbap.domain`·`com.kbap.application`을 쓰지 않는다. JPA 엔티티가 곧 도메인 모델이고(별도 도메인 모델·toDomain/from 변환 없음), **비즈니스 로직은 도메인 모델과 도메인 서비스가 소유**한다 — 도메인 서비스는 현재 전부 `com.kbap.api.<feature>` 소속이며, api 밖(배치) 소비자가 생길 때만 `:common` 으로 승격한다(ADR-0016 배치 기준). 도메인 경계는 모듈이 아니라 **패키지 + ArchUnit**(도메인 간 허용 방향 맵 — `ModuleBoundaryTest`)이 긋는다. 리포지토리·엔티티는 **public**(KB-220, ADR-0014) — 도메인 로직은 도메인 서비스를 경유하고, 단순 영속 접근은 소비 계층이 리포지토리를 직접 쓴다(위임 전용 창구 서비스 금지). 외부 시스템(jjwt·firebase·Redis·LLM·S3·SQS·Kakao)은 **인터페이스(seam)는 `common.port.*` 에, 구현은 소비 모듈의 어댑터 패키지(batch 도 쓰면 `common.infra.*`, api 전용이면 `api.infra.*`)에, 조립은 config 패키지** 패턴으로 격리한다(ADR-0018) — 어댑터 구현 직접 참조는 조립 config 와 어댑터 자신에서만 허용(ArchUnit 강제). DB 는 공유(batch `flyway off`, 스키마 owner=api). 공통 빌드 설정은 `buildSrc` 컨벤션 플러그인에 둔다.

### 모듈 구조

모듈 의존은 단방향으로 고정한다: **api·batch → `:common`**. api 와 batch 는 서로를 모른다.

```
:common  ← :api   (web bootJar, Flyway owner — 기능별 HTTP 경계·유스케이스 조합 + api 전용 어댑터 api.infra.*)
         ← :batch (배치 bootJar, flyway off — common 의 레포·어댑터를 직접 조립)
```

- `:common`: **영속 계층 전부 + api 밖(배치)이 컴파일 의존하는 코드**. 패키지 루트 `com.kbap.common`:
  - `common.core` — 도메인 소속이 아닌 공통뿐: **통합 에러**(`ErrorCode` + `BusinessException`)·테스트픽스처(`testsupport` — Testcontainers). Spring-free 유지.
  - `common.domain.<context>` — **전 컨텍스트의 엔티티(`model/`)와 리포지토리**(+JPQL projection dto — `FoodStatusCount` 등). 도메인 서비스는 여기 없다 — KB-301 이관으로 batch 의 공유 소비가 사라져 2026-08-14 전부 `com.kbap.api.<feature>` 로 내렸다. 미터링 이벤트 vocabulary(`LlmCallCostIncurred`)는 `common.domain.metering` 소속.
  - `common.util` — 순수 유틸(`ImageUrls`·`KoreanMenuNameNormalizer`). Spring-free.
  - `common.port.{llm,storage,auth,mq,place}` — **어댑터 계약(seam) 전부**: llm(`MenuBoardVisionExtractor`·`FoodImageBatchClient`·`TextEmbeddingClient`), storage(`StorageObjectStore`·`PresignedUploadPort`), auth(`TokenIssuer`·`TokenParser`·`SocialTokenVerifier`·`RefreshTokenStore`·`SocialAccountDeleter`), mq(`FoodContentEventPublisher`), place. 설정 홀더는 계약이 아니다 — jwt 설정(`JwtTokenProperties`)은 `api.infra.auth` 소속. 순수 계약 — Spring·JPA 를 모른다(ArchUnit 강제). 포트가 도메인 타입을 반환하는 방향은 허용, 도메인이 포트를 아는 역방향은 금지.
  - `common.infra.{llm,mq}` — **batch 도 쓰는 어댑터 구현**(ADR-0018): llm(Spring AI — vision·embedding·이미지 배치), mq(SQS 아웃박스 발행). api 전용 어댑터는 여기 두지 않는다.
- `:api`: web bootJar — `com.kbap.api.<feature>`에 controller·API DTO(`*Request`/`*Response`)·API 전용 서비스·결과 타입을 기능별로 함께 두고, `config/`가 빈을 조립한다. api 전용 어댑터 구현(jjwt+firebase·redis·S3·kakao)은 `com.kbap.api.infra.{auth,redis,storage,place}`(ADR-0018). `com.kbap.domain`·`com.kbap.application` 계층 패키지는 사용하지 않는다(ADR-0017). 진입점 `KbapApiApplication`은 `com.kbap` 루트(스캔이 전 계층 커버). DB 마이그레이션(Flyway) **스키마 owner** — `src/main/resources/db/migration`.
- `:batch`: 배치 bootJar. **컴포넌트 스캔을 자신 + `com.kbap.common.infra.llm` 로 좁힌다** — 도메인 서비스 그래프를 올리지 않고, common 의 리포지토리·어댑터를 직접 주입해 조립한다(트랜잭션 경계는 배치가 `TransactionTemplate` 로 소유). 엔티티/레포 스캔은 `@AutoConfigurationPackage("com.kbap")`. **flyway off**(스키마 owner=api). 패키지 `com.kbap.batch`.

도메인 패키지는 `com.kbap.common.domain.<context>`로 일원화한다 — **엔티티(=도메인 모델, 도메인 메서드 내장) + 리포지토리(public)**를 소유한다. 도메인 서비스·요청 조합·외부 seam 호출·응답 조립은 `com.kbap.api.<feature>`에 두고, web·batch 가 공유하게 되는 비즈니스 로직만 common 의 도메인 서비스로 승격한다(현재 해당 없음). 도메인 간 의존은 단방향만 허용하며 `ModuleBoundaryTest`가 `common.domain` 패키지 간 방향을 검사한다. API 기능 패키지는 컨트롤러와 유스케이스 조합을 함께 가지므로 이 도메인 맵의 대상이 아니다.

`:common`→data-jpa 는 `api`(엔티티가 서비스 시그니처에 노출), 그 외 모듈 간 의존은 `implementation` 기본. 경계 강제는 — **Gradle**(api↔batch 상호 참조 차단), **ArchUnit**(`ModuleBoundaryTest.kt`, Kotest 태그 `arch` — `common.domain` 간 허용 방향 맵·커널 Spring-free·도메인→api/어댑터 금지·어댑터 구현 참조는 조립 창구만·`@Entity` 위치·API 구 패키지 금지). 도메인 로직의 도메인 서비스 소유는 규율·리뷰로 지킨다(ADR-0014).
...
- **Spring Boot 4.1** — web/validation/actuator/data-jpa/data-redis 스타터. 영속: **MySQL**(prod, `mysql-connector-j`, 통합 테스트는 MySQL Testcontainers) + **Redis**(refresh token — KB-118). DB 마이그레이션: **Flyway**(+flyway-mysql). API 문서: **springdoc-openapi**(Swagger UI). 인증: 자체 JWT(jjwt) + Firebase ID 토큰 검증(firebase-admin) — 구현은 `api.infra.auth`, refresh token 저장은 `api.infra.redis`.
- **LLM: Spring AI 2.0**(Boot 4 호환 라인) — 어댑터 패키지 **`common.infra.llm`**(ADR-0018, 구 `:infra:llm`)에 `spring-ai-starter-model-openai` 하나(비전 `OpenAiChatModel`·임베딩 `OpenAiEmbeddingModel` — 이미지 배치만 OpenAI Batch API 직접 호출). seam 은 `common.port.llm` 3종(`MenuBoardVisionExtractor`·`FoodImageBatchClient`·`TextEmbeddingClient`)뿐이고 구현·구성이 이 패키지에 응집된다. **소비자는 `:api`·`:batch`(벡터 동기화, KB-328)**다 — 음식 콘텐츠 채움이 kbap-langchain 으로 이관되면서(KB-301) 배치용 채팅 caller·fan-out·다중 벤더가 전부 제거됐다(KB-320). `kbap.llm.*` 프로퍼티 + `@ConditionalOnProperty`로 명시 구성한다: `vision`(메뉴판 스캔, `gpt-5.6-luna`)·`image`(`gpt-image-2` 배치 이미지 생성)·`embedding`(OpenAI `text-embedding-3-small` 256차원 — KB-328 에서 Bedrock Titan 전환, `OPENAI_API_KEY` 재사용). api 는 `vision.enabled: true` 고정 — 스캔이 필수 기능이라 `OPENAI_API_KEY` 없이는 부팅하지 않는다. vision 전용 환경변수는 없다(사진 fetch 도메인은 `kbap.storage.public-base-url` 을 참조). Spring AI 자동구성 유입은 `application.yml`의 `spring.ai.model.*=none`으로 차단.
...
- **공통 설정은 `buildSrc` 의 컨벤션 플러그인**(미리 컴파일된 `kbap.*.gradle.kts`)에 둔다. 각 모듈은 `plugins { id("kbap.<archetype>") }` 한 줄로 자기 아키타입을 선언한다. 루트 `build.gradle.kts`는 거의 비어 있다(집계 전용).
  ...
  - `kbap.spring-conventions` — **Spring 공통 베이스**(단독 사용 모듈 없음 — spring-boot-application 의 토대): kotlin-common 위에 kotlin-spring·dependency-management·Spring Boot/AI BOM·`kotlin-reflect`/`jackson-module-kotlin`/`spring-boot-starter-test`를 얹는다.
...
- **모듈별 고유 설정만 각 모듈 `build.gradle.kts`** 에 둔다(api=web/validation/actuator/flyway/springdoc+kotlin-jpa+어댑터 SDK(jjwt·firebase·redis·s3), common=spring-ai+sqs(+AI BOM import), batch=sqs 등). 모듈 build 파일에서 의존성은 **문자열 표기**(`"implementation"(...)`)로 적는다(플러그인이 컨벤션에서 적용돼 타입 안전 단축표기 미생성). 라이브러리 좌표는 모듈 build 파일에선 `libs.*`로 정상 사용.
...
- 소스는 각 모듈의 `src/main/kotlin/...`, 테스트는 `src/test/kotlin/...`에서 동일 구조로 미러링한다. **패키지는 모듈 경로를 미러링한다** — `:common`은 `com.kbap.common` 아래에 커널 `core`(에러·테스트픽스처), 유틸 `util`, 도메인 `domain.<context>`, 외부 시스템 seam `port.{llm,storage,auth,mq,place}`, 공유 어댑터 구현 `infra.{llm,mq}`를 둔다. `:api`는 `com.kbap.api.<feature>` 기능 패키지에 controller·request/response·서비스·결과 타입을 함께 두며, 파일 수가 적은 기능에 `service`·`dto` 하위 패키지를 만들지 않는다. api 전용 공통재(BaseResponse·ApiPaths·예외핸들러·인증 부품·로깅)는 `com.kbap.api.core`, 빈 조립은 `com.kbap.api.core.config`, api 전용 어댑터 구현은 `com.kbap.api.infra.<어댑터>`에 둔다. 배치는 `com.kbap.batch`다. **부트 진입점 `KbapApiApplication`은 패키지 루트 `com.kbap`** 에 두어 기본 컴포넌트 스캔·AutoConfigurationPackages 가 전 계층(엔티티·리포지토리 포함)을 커버한다(별도 `scanBasePackages` 불필요). 배치 진입점은 `com.kbap.batch` — 단 배치는 `scanBasePackages` 를 자신 + `com.kbap.common.infra.llm` 로 좁힌다(도메인 서비스 미탑재).
...
- **테스트 스타일 (고정).** **모든 테스트는 Kotest `BehaviorSpec` 으로 통일**한다(다른 Spec 스타일·JUnit `@Test` 금지). 구조는 **`given("대상/전제") > \`when\`("상황") > then("기대 결과")`** 를 기본으로 하며, `given`·`` `when` ``·`then` 설명은 **한국어**로 쓴다(예: `` given("BoundingBox 생성") { `when`("x 가 음수이면") { then("예외를 던진다") { ... } } } ``).
  ...
  - MockMvc 는 `@Autowired MockMvc` 로 주입만 한다 — 자동 구성은 아래 `@IntegrationTest` 가 담당(`ObjectMapper` 빈은 주입 안 되므로 `jacksonObjectMapper()` 로 직접 생성).
  - **통합 테스트 헤더는 합성 애너테이션 하나로 고정한다 (2026-08-29, KB-392).** api 는 `@IntegrationTest`(`com.kbap.api` — `@SpringBootTest` + `@AutoConfigureMockMvc` + MySQL·Redis 픽스처 + 소셜 인증·장소 검색 페이크 `@Import`), batch 는 `@BatchIntegrationTest`(`com.kbap.batch` — + `SlowJobTestConfig`), common 리포지토리 테스트는 `@SpringBootTest` + `@Import(MySqlContainerConfig::class)` 에 부트 클래스 `CommonTestApp`(도메인 패키지만 스캔) 하나. **테스트 클래스에 `@SpringBootTest(...)`·`@AutoConfigureMockMvc`·`@Import(...)`·`properties` 를 직접 쓰지 않는다** — 설정 조합이 한 글자라도 다르면 Spring TestContext 가 새 컨텍스트(= 새 MySQL 컨테이너 + Flyway 재실행)를 만들어 캐시에 살려 두기 때문이다(통합 전 api 8·common 7·batch 3 → 후 1·1·1). 프로퍼티가 필요한 검증(구조화 로깅 등)은 컨텍스트를 새로 띄우지 말고 Spring 없이 대상 컴포넌트를 직접 구성해 검사한다(`StructuredConsoleLoggingTest` — `StructuredLogEncoder` 직접 구성; Boot 의 `LogbackLoggingSystem` 은 JVM 당 한 번만 초기화되므로 두 번째 `SpringApplication` 은 로깅 프로퍼티를 반영하지도 않는다). 페이크는 모든 api 통합 컨텍스트에 기본 포함되므로 상태는 `beforeSpec` 에서 `reset()` 한다. **한 컨텍스트 = 한 DB 를 전 클래스가 공유**하므로 클래스는 자기 시드를 자기가 만들고, 전체 정리는 `TestTables.clearAll(dataSource)`(FK 검사 끄고 카탈로그 `ingredients`·`app_version` 제외 전 테이블 삭제)로 한다 — 부분 `DELETE FROM food/member` 는 다른 클래스가 남긴 자식 행에 FK 로 막힌다. `ingredients` 를 비운 뒤 카탈로그가 필요하면 `IngredientTestSeed.restoreCatalog`.
...
- **벡터 스토어 = id 만 반환 (고정 — 2026-09-03, KB-413).** 벡터 검색은 id(+score)만 돌려주고, 표시 내용은 항상 MySQL 재조회로 얻는다. **벡터 메타데이터에 표시용 필드를 넣지 않는다** — 넣어야 한다면 변경감지 해시에 포함해 stale 을 막는다(안 그러면 원본 변경이 재업서트를 트리거하지 않아 오래된 값이 영구 잔존한다).
- **LIKE 검색 키워드는 `LikeWildcards.escape` 경유 + 쿼리에 ESCAPE 절 필수 (고정 — 2026-09-05, KB-416).** `@Query` 의 LIKE(네이티브·JPQL 모두)는 파라미터 바인딩만으로 `%`·`_` 가 이스케이프되지 않는다 — 호출자는 `common.util.LikeWildcards.escape` 로 키워드를 이스케이프하고, 쿼리에는 ESCAPE 절(네이티브 `ESCAPE '\\'`·JPQL `escape '\'`)을 단다. 파생 쿼리(`Containing`)는 Spring Data 가 자동 이스케이프하므로 대상 밖. `RepositoryLikeEscapeTest`(arch) 가 like 절 수 = escape 절 수를 강제한다.
...
- **common 엔티티 스키마 변경은 마이그레이션 선행 PR (고정 — 2026-09-08, KB-446/KB-451/KB-459).** `common.domain` 엔티티에 컬럼을 추가·변경하는 작업은 **① Flyway 마이그레이션만 담은 PR(`api/**` 만 건드려 batch 배포를 트리거하지 않음)을 먼저 머지하고 dev·prod 각 환경에 실제 적용된 것을 확인한 뒤 → ② 엔티티·기능 PR** 순서로 나눈다. 이유: develop 푸시 시 API·batch 배포가 동시에 도는데 batch 는 Flyway 없이 `ddl-auto=validate` 만 하므로, 엔티티가 매핑한 컬럼이 아직 없는 환경에서 신 코드 batch 태스크가 기동 실패한다(구 코드+신 스키마는 잉여 컬럼 허용이라 무해 — 역방향만 위험). ②에는 ①과 **동일한 마이그레이션 파일(같은 버전·내용)** 을 함께 담아 develop 기준 테스트가 통과하게 하고(어느 순서로 머지돼도 안전), 두 PR 본문에 머지 순서를 명시한다. 백필·데이터 정정만 있는 마이그레이션(스키마 무변)은 batch validate 와 무관하니 선행 분리 대상이 아니다.
