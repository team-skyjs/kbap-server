...
`specs/kb-117-member-schema-consolidation/plan.md` (KB-117 member 스키마 재편 — `member_social_identities` 를 DROP 하고 신원 3컬럼(provider ENUM·provider_uid·email)을 `member`(구 `members` 단수형 리네임) 행으로 흡수, `(provider, provider_uid)` 유니크는 member 에 유지(로그인 "조회→없으면 가입" 단일 분기 — 동시 첫 로그인 경합은 발생 확률 희박으로 범위 제외). **탈퇴 = 소프트 삭제 + `provider_uid="DELETED:{memberId}"` 삭제 표식 치환(email·provider 는 보존)** — 유니크 유지한 채 재가입 개방. 표식에 memberId 를 붙이는 건 필수 — 상수 `DELETED` 만 쓰면 같은 provider 의 두 번째 탈퇴가 유니크 충돌로 실패한다. **정지는 `member_status ENUM('ACTIVE','SUSPENDED')` 별도 컬럼** — BaseEntity.status 에 SUSPENDED 추가 금지(@SQLRestriction 이 관리자 조회까지 숨김). 서비스 조회는 파생 쿼리(findByIdAndMemberStatus·findByProviderAndProviderUidAndMemberStatus)로 ACTIVE 만, 관리자 조회는 JPA 상속 findById(무필터). fetch join 2개 제거. 프로필 4항목(기피성분·맵기·국가·언어)은 `profile` JSON 단일 컬럼(영속 전용 `MemberProfileJson` 스냅샷, nickname 은 컬럼 유지). `onboarding_status` 는 BOOLEAN(TINYINT(1)) 저장 — 도메인 `OnboardingStatus` enum 유지, 엔티티 Boolean↔enum 변환. 도메인 `Member` 는 identities(List)→단일 identity 축소(계정 연결 포기), `SocialIdentity`·`MemberRepository`·`MemberIdentityResolver` 불변 — port 시그니처 무변경(`MemberProfile` 만 2건 수정 — countryCode 는 kernel 신규 `CountryCode` enum(ISO alpha-2 197개국 + 한국어 label, 소스 specs/kb-117.../countries.json) 타입 `CountryCode?` 로 — 소비처는 리뷰 국적 필터(review 행 스냅샷 예정)·표시명은 클라이언트 소유, spicinessPreference 비-널 `Int`·`empty()` 초기값 5(UI 기본 제안값 일치, null 상태 소멸 — 마이그레이션 백필 `COALESCE(...,5)`)). `MemberStatus` enum 은 영속 모듈(도메인 미보유). 마이그레이션 1건: RENAME→컬럼 추가→JOIN 백필→신원 없는 과거 탈퇴 행은 `DELETED:{id}` 백필→onboarding BOOLEAN 변환→NOT NULL·유니크 승격→구컬럼·구테이블 DROP. ENUM·유니크는 엔티티 columnDefinition·uniqueConstraints 에도 선언(영속 테스트 스키마 = 엔티티 생성). Flyway 는 테스트 미커버 — 로컬 docker MySQL DROP+CREATE 후 bootRun 으로 검증.)
<!-- SPECKIT END -->
