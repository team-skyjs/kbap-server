...
`specs/kb-138-menu-photo-scan/plan.md` (KB-138 메뉴판 사진 스캔 — 업로드 완료 검증 + gpt-4o-mini 메뉴명·가격 추출. 기존 OCR 텍스트 스캔을 **이미지 path 기반으로 대체**(공존 없음). 두 API: (1) `POST /api/v1/images/complete` — presigned 업로드(KB-145, 발급은 범위 밖) 후 완료 신고, 서버가 HeadObject 로 실제 Content-Type(`image/*` 만 허용, 크기 상한 없음)·크기를 신고값과 대조, 실패 시 DeleteObject + `IMAGE-001~003`, 성공 시 `uploaded_image` 기록(같은 path 재신고 멱등). (2) `POST /api/v1/scans` — 요청 `{imagePath}`(path only, 전체 URL 거절), 검증·본인 소유 확인(`SCAN-001`) 후 vision 추출 → 기존 food 매칭·회피성분 위험도 판정 연결, 응답은 **기존 구조 additive**(항목별 `price` 추가, idx=서버 부여 추출 순번, 미매칭도 vision name/koreanName 채움, degraded 상수 false, vision 실패는 `SCAN-002` 503). 신규 모듈 2: **`:domain:image`**(리프 — `UploadedImage` 엔티티·`ImageUploadService` 창구·`StorageObjectStore` seam, `image ← scan` 단방향)와 **`:infra:storage`**(AWS SDK v2 S3 head/delete 구현, 조립은 `app/api/config/StorageConfig` `@ConditionalOnProperty(kbap.storage.enabled)` — KB-145 presign 도 향후 이 모듈). vision 은 `:core` seam `MenuBoardVisionExtractor` + `:infra:llm` 의 `OpenAiMenuBoardVisionExtractor`(전용 프로퍼티 `kbap.llm.vision.*` — 배치 채점 모델과 독립, Spring AI `Media`(URI)로 이미지 URL 직접 전달 = 바이트 서버 무경유, `image-base-url`+path 조합은 구현 책임, 응답은 `responseFormat`(JSON) 강제 — 스파이크 프롬프트·천원 축약 가격 복원 지침 이식, 파싱은 `MenuBoardResultParser` 단위 검증). `scan_history` 단일 테이블 확장(image_path·menu_name·korean_name·price 추가, food_id nullable 완화 — 미매칭도 기록, `findRecentReadyFoodIds` 에 `IS NOT NULL`), **가격 저장은 스캔 히스토리 한정**(food 마스터 금지). Flyway 2건(uploaded_image 생성·scan_history 확장). 외부 호출(Head/Delete/vision)은 전부 트랜잭션 밖 — 짧은 tx 로 검증→기록, 추출→저장 분리. Test-First: 도메인 단위(페이크 seam)·파서 단위·MockMvc 통합(@TestConfiguration 페이크 빈) — 실 S3·실 OpenAI 무접근.)
