...
`specs/kb-145-presigned-url/plan.md` (KB-145 이미지 업로드용 S3 presigned URL 발급 API — 인증 사용자에게 **업로드용 presigned PUT URL** + 저장·표시용 **안정 공개(CDN) URL** 을 함께 발급하는 단일 엔드포인트 `POST /api/v1/images/upload-url`. 범용 이미지 업로드 창구(용도 purpose 로 구분, 첫 소비자=메뉴판 스캔). **핵심 성질: SigV4 presign 은 로컬 서명이라 발급 시 S3 미호출 — DB·Flyway·엔티티·네트워크 왕복 전무.** auth 패턴 미러링: 무소속 유스케이스 `ImageUploadApplicationService` + seam `PresignedUploadPort` 를 :application 에, S3 어댑터 `S3PresignedUploadPort` 를 **신규 모듈 :infra:storage** 에, 컨트롤러·DTO·빈 조립을 :app:api 에. 구현 축: (1) 서명=**Presigned PUT**(AWS SDK v2 `s3` 만 의존 — presigner 클래스가 s3 아티팩트에 포함, BOM 2.33.2) — v2 는 POST policy 미제공이라 PUT 에 **정확 Content-Length(클라 신고값)+Content-Type** 을 서명해 크기 상한 강제(DoD content-length-range 의도 충족, UPLOAD-003 발급 거절). (2) 읽기=**만료 없는 공개/CDN URL** `{public-base-url}/{objectKey}` — 별도 조회 서명·재발급 없음(사용자 확정, KB-147 CloudFront 방향 일치). (3) 객체 키 `{purpose-prefix}/{yyyy}/{MM}/{dd}/{memberId}/{UUID}.{ext}` — 충돌 불가·추측 불가. (4) 부팅 안전=auth 의 UnavailableSocialAuth 패턴 — :infra:storage 가 @ConditionalOnProperty("kbap.storage.bucket") 로 실 port, :app:api 가 @ConditionalOnMissingBean 로 UnavailablePresignedUploadPort fallback(local/test 는 버킷 없이 부팅, 실 S3 는 dev/prod 만). (5) :core ErrorCode 에 UPLOAD-001(형식)·002(용도)·003(초과) 추가. Test-First: `ImageUploadApplicationServiceTest`(페이크 port 로 검증·키 생성, S3 무관)=핵심 Red + `ImageUploadControllerTest`(MockMvc 401·400·200) + `S3PresignedUploadPortTest`(경량, 더미 정적 자격증명). 신규 의존성 1(AWS SDK v2), 마이그레이션 0, :app:batch·도메인 모듈 범위 밖. **현재 이 작업은 develop 기준 git worktree `.claude/worktrees/kb-145-presigned-url` 브랜치에서 진행.**)

이전 플랜: `specs/kb-153-food-bookmark-flag/plan.md` (KB-153 음식 리스트·상세 조회 응답에 북마크 여부(bookmarked) 포함 — 비회원 항상 false, 회원은 실제 북마크 여부. **조합 지점은 :app:api 컨트롤러** — :domain:bookmark 가 :domain:food 에 의존하므로(북마크 목록=음식 요약 재사용) food 쪽에 북마크 조회를 넣으면 Gradle 순환. 구현 축: (1) `BookmarkService.findBookmarkedFoodIds(memberId: Long?, foodIds): Set<Long>` 신규 — null 회원/빈 집합이면 쿼리 없이 emptySet(비회원 false 규칙이 서비스 한 곳에), 내부는 `BookmarkJpaRepository.findByMemberIdAndFoodIdIn` derived query(internal, 소프트삭제는 @SQLRestriction 자동 제외), 페이지당 IN 쿼리 1회로 N+1 없음. (2) API 응답 DTO 에만 필드 추가 — `FoodSummaryResponse`(리스트·검색·북마크 목록 공유)·`FoodDetailResponse` 에 `bookmarked: Boolean`(non-null, from(...) 파라미터 기본값 없음 — 병합 누락을 컴파일이 잡게). 도메인 dto(FoodSummaryView·GetFoodDetailResult) 무변경. (3) 컨트롤러 병합 4곳 — FoodController browse·search·detail(검색은 DTO 구조 공유의 자연 결과로 동일 규칙), BookmarkController 목록은 정의상 **상수 true**(쿼리 낭비 금지). (4) Swagger — "비회원은 항상 false" 명시. Test-First: MockMvc 통합(비회원 false·회원 true/false 혼재·취소 후 false·목록 true, 인증은 BookmarkControllerTest 의 TokenIssuer 헬퍼 재사용)이 Red 진입점 + BookmarkServiceTest 에 창구 검증(null 회원·소프트삭제 제외·교집합 반환). DB·Flyway·엔티티·모듈 의존 그래프 무변경, 신규 의존성 0, :application 승격 불필요(순환 아님·부트앱 레벨 병합). :app:batch 범위 밖.)

이전 플랜: `specs/kb-147-profile-image-url/plan.md` (KB-147 프로필 사진 URL·맵기 선호 필드 추가 — 온보딩 요청·프로필 부분 수정 요청·내 프로필 조회 응답에 `profileImageUrl` 과 `spicinessPreference`(기존 도메인 속성 0~10·기본 5, 현재 입출력이 막혀 있어 개방 — DTO 둘 다 `Int? = null` 선택 입력, 미전송=기존 값 유지, 범위 밖은 `validatedSpiciness` 가 신규 `INVALID_SPICINESS_PREFERENCE`("MEMBER-009", 400) 로 선행 거절, `MemberProfile` init require 는 최후 방어선) 추가. **저장은 기존 `member.profile` JSON 컬럼의 `MemberProfileJson` 필드 추가 — Flyway 마이그레이션 0건**(기존 row 는 키 부재 → Jackson 기본 null = 미설정, Jira DoD 의 "nullable 컬럼" 의도 충족). 부분 수정 3분법: 미전송(null)=유지 · 값=검증 후 교체 · **빈 문자열=제거**(별도 삭제 API 없음, 사용자 확정). 검증은 `MemberService` 의 기존 `validated~` 관례로 `validatedImageUrl` — https 스킴 + URI 파싱 + 호스트 존재 + 길이 ≤512 + **허용 호스트 목록 `kbap.member.profile-image-allowed-hosts`**(콤마 구분, `@Value` 생성자 주입, 빈 값(기본·local·테스트)이면 형식 검증만, prod 는 CloudFront CDN 호스트 등록 — 사용자 확정) 불합격 시 신규 `ErrorCode.INVALID_PROFILE_IMAGE_URL`("MEMBER-008", 400). 변경 모듈 3개(:core ErrorCode·:domain:member·:app:api), 신규 파일 0건 — MemberProfile/MemberProfileJson/MemberProfileInput/ProfileUpdateInput/MyProfileResult + OnboardingRequest/ProfileUpdateRequest/MyProfileResponse/MemberApi 에 필드 추가, 신규 엔드포인트·DB 스키마 변경 없음. 테스트: MemberServiceTest(도메인 통합, 온보딩 등록/미등록·수정 교체/유지/제거·검증 거절) + MemberControllerTest(MockMvc) — Kotest BehaviorSpec 한국어 given/when/then, Red→Green 테스트 우선. presigned URL 발급·이미지 업로드는 별도 태스크로 범위 밖.)
