    ...
    given("정제 서비스가 정상일 때") {
        `when`("표준명이 완성된 저장 음식과 일치하면") {
            then("MATCHED 이고 산출된 위험도를 반환한다") {
                ...
                val result = uc.scan(MenuScanInput(listOf(item(0, "김치찌개 kimchi jjigae"))))
        ...
        `when`("표준명이 저장에 없으면") {
            then("food 에 미완성으로 등록하고 PENDING·UNKNOWN 으로 응답한다") {
                ...
                val result = uc.scan(MenuScanInput(listOf(item(0, "우주라면"))))
        ...
        `when`("매칭되지 않은 항목이 여러 개면") {
            then("이름을 모아 저장소를 한 번만 호출한다(항목마다 반복 저장하지 않는다)") {
                ...
                val result = uc.scan(
                    MenuScanInput(listOf(item(0, "우주라면"), item(1, "탕후루"), item(2, "우주라면 space"))),
                )
        ...
        `when`("모든 항목이 저장 음식과 매칭되면") {
            then("저장소에 빈 집합을 넘겨 새 음식을 만들지 않는다") {
                ...
                uc.scan(MenuScanInput(listOf(item(0, "김치찌개"))))
        ...
        `when`("LLM 이 NOT_FOOD 로 판정하면") {
            then("결과에서 제외되고 food 를 만들지 않는다") {
                ...
                val result = uc.scan(MenuScanInput(listOf(item(0, "원산지 중국"))))
        ...
        `when`("한글이 전혀 없는 항목이면") {
            then("LLM 을 거치지 않고 결과에서 제외한다") {
                ...
                val result = uc.scan(MenuScanInput(listOf(item(0, "MacBook Air F9"))))
        ...
        `when`("여러 항목을 제출하면") {
            then("스캔당 LLM 을 1번만 호출한다") {
                ...
                uc.scan(MenuScanInput(listOf(item(0, "김치찌개"), item(1, "원산지 중국"))))
        ...
        `when`("한 스캔 안에서 같은 표준명이 여러 항목으로 나오면") {
            then("미완성 음식을 1번만 생성한다") {
                ...
                uc.scan(MenuScanInput(listOf(item(0, "우주라면"), item(1, "우주 라면"))))
    ...
    given("정제 서비스 호출이 실패할 때(폴백)") {
        `when`("interpreter 가 예외를 던지면") {
            then("아는 메뉴는 MATCHED, 나머지는 food 생성 없이 PENDING 으로 강등한다") {
                ...
                val result = uc.scan(MenuScanInput(listOf(item(0, "김치찌개"), item(1, "우주라면 space"))))
        ...
        `when`("interpreter 가 요청보다 적은 개수를 반환하면") {
            then("결과를 신뢰하지 않고 정규화 exact 매치 폴백으로 degraded=true 를 알린다") {
                ...
                val result = uc.scan(MenuScanInput(listOf(item(0, "김치찌개"), item(1, "우주라면"))))
    ...
    given("응답 메뉴명 — 현재는 한국어 고정(회원 언어 설정 연동 전)") {
        `when`("매칭된 음식에 en 번역이 있어도") {
            then("name 을 한국어로 내리고 koreanName 과 같다") {
                ...
                val result = uc.scan(MenuScanInput(listOf(item(0, "김치찌개 kimchi"))))
        ...
        `when`("폴백 중 아는 음식이 아니어서 판정이 불가하면") {
            then("서버가 아는 이름이 없으므로 name·koreanName 이 null 이다") {
                ...
                val result = uc.scan(MenuScanInput(listOf(item(0, "우주라면 space"))))
