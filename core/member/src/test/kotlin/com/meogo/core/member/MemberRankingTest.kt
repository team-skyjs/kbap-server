...
class MemberRankingTest : BehaviorSpec({

    given("랭킹 점수 산출") {
        `when`("리뷰 8건·고유 음식 6종·스캔 9회이면") {
            then("점수는 128점이고 등급은 explorer 이며 다음 등급까지 52점 남는다") {
                val ranking = MemberRanking.of(reviewCount = 8, uniqueReviewedFoodCount = 6, scanCount = 9)
        ...
        `when`("활동이 전혀 없으면") {
            then("점수는 0점이고 최하 등급이며 다음 등급까지 30점 남는다") {
                val ranking = MemberRanking.of(reviewCount = 0, uniqueReviewedFoodCount = 0, scanCount = 0)
        ...
        `when`("스캔만 했으면") {
            then("스캔 1회당 2점만 쌓인다") {
                MemberRanking.of(reviewCount = 0, uniqueReviewedFoodCount = 0, scanCount = 7).score shouldBe 14
            }
        }
        ...
        `when`("카운트가 음수이면") {
            then("예외를 던진다") {
                shouldThrow<IllegalArgumentException> {
                    MemberRanking.of(reviewCount = -1, uniqueReviewedFoodCount = 0, scanCount = 0)
                }
            }
        }
    }
    ...
    given("최고 등급 도달") {
        `when`("점수가 1000점 이상이면") {
            then("다음 등급과 남은 점수가 없다") {
                val ranking = MemberRanking.of(reviewCount = 0, uniqueReviewedFoodCount = 0, scanCount = 600)
