...
interface NotificationSettingApi {
    @Operation(
        ...
        description = """
            ...
            **활동 푸시** — `activity`: 활동/소식(리뷰 도움됨·리뷰 작성 리마인더) 토글 하나. 설정 기록이 없으면 `true`.

            **K-Bap에서 보내는 소식(광고성 푸시)** — `kbapNews.enabled` 는 저장값이 아니라 「마케팅 목적 개인정보 수집·이용 동의」와
            「광고성 정보 수신 동의」가 **모두 유효한지**로 계산한다. `kbapNews.mealTime`(식사 시간 알림, 점심·저녁 넛지)은
    ...
    @Operation(
        ...
        description = """
            ...
            처리 순서는 `activity` → `kbapNews.enabled` → `kbapNews.mealTime` 이다.

            **`kbapNews.enabled: true`(켜기)** — `privacyConsentVersion`·`receiveConsentVersion` 이 **모두 필수**(400 COMMON-002).
            종류별로 같은 버전의 열린 동의가 있으면 무변화, 다른 버전이면 그 동의를 철회한 뒤 새 동의를 남긴다(재동의·문구 개정).
            동의 시각은 서버가 찍고 클라이언트 값은 무시한다. 식사 시간 알림은 저장값(처음이면 켜짐)이 그대로 복원된다.

            **`kbapNews.enabled: false`(끄기)** — 두 종류의 열린 동의를 전부 철회한다(행 보존). 식사 시간 알림 저장값은 남는다.

            **`kbapNews.mealTime: true`** — 소식이 꺼져 있으면(동의 미완) 400 `NOTIFICATION-001`. `false` 는 항상 허용.
