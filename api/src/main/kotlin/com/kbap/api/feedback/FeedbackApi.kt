...
interface FeedbackApi {
    ...
    @Operation(
        ...
        description = """
            범위는 신원에 따라 갈린다 — **로그인 상태면 그 회원이 보낸 문의만**(기기 무관, 다른 기기에서 로그인해도 보인다),
            **비로그인이면 그 기기에서 게스트로 보낸 문의만**이다. 기기를 공유해도 회원 문의가 게스트에게 보이지 않고 그 반대도 없다.
            게스트로 보낸 문의를 로그인 뒤에 이어 보는 것(계정 이전)은 지원하지 않는다.
            답변은 오래된 순이며 답변자 정보는 내려주지 않는다.
    ...
    @Operation(
        ...
        description = """
            ...
            소유 조건은 **목록과 같은 범위**(회원이면 회원 id, 비로그인이면 그 기기의 게스트 문의)이며,
            범위 밖이면 존재 여부를 숨기고 404(FEEDBACK-004)로 답한다.
        """,
    )
    @ApiResponses(
        value = [
