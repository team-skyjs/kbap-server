...
interface PlaceApi {
    @Operation(
        summary = "주변 식당 검색",
        description = """
            리뷰 작성 화면에서 사용자 위치(위도·경도) 기준으로 가까운 식당을 최대 5건 반환한다(가까운 순, 페이징 없음).
            검색 키워드는 "음식점" 으로 서버가 고정하며, 외부 지도 검색을 대신 호출하므로 클라이언트는 검색 자격증명을
            갖지 않는다. 결과 항목(items[])을 그대로 리뷰 작성 요청의 place 로 보내면 된다. 결과가 없으면 빈 배열을 반환한다(오류 아님).
        """,
    )
    @ApiResponses(
        value = [
