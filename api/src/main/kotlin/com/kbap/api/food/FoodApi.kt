...
interface FoodApi {
    ...
    @Operation(
        summary = "스캔 음식 목록 조회 (리뷰 태그 후보, 회원 전용)",
        description = """
            본인 스캔 이력에 매칭된 음식을 중복 없이 마지막 스캔 시점 내림차순으로 한 페이지 20개씩 조회한다.
            리뷰 작성 화면의 음식 태그 후보 용도다 — 같은 음식을 여러 번 스캔했어도 한 번만 나오고, 재스캔하면 맨 앞으로 올라온다.
            삭제·비공개 음식과 음식 마스터에 매칭되지 않은 스캔 항목은 제외되며, 스캔 이력이 없으면 빈 목록이다.

            keyword(옵션)를 주면 목록 안에서 음식명으로 거른다 — 매칭 규칙은 음식 검색과 동일(한국어명 또는 요청 언어 번역명 부분 일치,
            대소문자 무시)하고, 본인 스캔 음식 범위를 벗어나지 않는다. 빈/공백 keyword 는 400 이다.
    ...
    @ApiResponses(
        value = [
            ...
            ApiResponse(responseCode = "400", description = "빈/공백 keyword, 잘못된 커서(형식·본인 스캔 이력에 없음), 또는 lang 누락·빈/공백"),
