...
interface FoodApi {
    ...
    @Operation(
        ...
        description = """
            ...
            리뷰 요약은 review 중첩 객체로 내려간다 — review.averageRating(전체 평균, 소수 첫째 자리 반올림)·review.reviewCount·
            review.sameCountryAverageRating(같은 국적 평균, 작성 시점 국적 스냅샷 기준)·review.blur. 수치는 null 없이 항상 숫자이며 해당 값이 없으면 0.0 이다.
            blur 는 활성 회원 조회면 false, 비회원(또는 탈퇴 회원 토큰) 조회면 true 이고 이때 수치는 전부 기본값(0.0·0)이다.
            클라이언트 판별 규칙: blur=true → 로그인 유도 가림 상태, blur=false && reviewCount==0 → 리뷰 없음 상태.
