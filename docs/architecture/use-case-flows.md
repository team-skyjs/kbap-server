...
- **컨텍스트 간 조합은 `meogo-application`에서만** 일어난다. 도메인 컨텍스트끼리 직접 호출하지 않는다.
...
표준 participant 이름(다이어그램 전반 고정): `User` · `Client` · `meogo-api`(API) · `meogo-application`(App) · `scan` · `food` · `member` · `assessment`(Assess) · `research`(Research) · `meogo-infra·LLM` · `meogo-infra·Email`. 배치는 `meogo-batch`(Batch).
...
- **트리거**: 앱 설치 후 첫 가입
- **사용 컨텍스트**: `member`(Identity/Auth + 프로필 + 식이 제한) + `meogo-infra·Email`
...
sequenceDiagram
    ...
    participant App as meogo-application
    participant Member as member
    participant Email as meogo-infra·Email
...
- **사용 컨텍스트**: `research`(조사 대기열·종합 정책) → `food`(영속) → (`meogo-infra·LLM`). 조율은 `meogo-application`의 **배치 전용 유스케이스**, 트리거는 `meogo-batch`([ADR-0004](../adr/0004-research-bounded-context.md))
...
4. **온보딩 이메일 인증 어댑터** — `meogo-infra·Email` 은 문서에 LLM 만큼 구체화돼 있지 않음(스택 문서엔 미등장). UC-5 는 합리적 추정.
