...
- LLM 외부 호출의 기술 구현(= `meogo-infra` 어댑터, core port 구현)
...
- `scan`: 직접 의존하지 않는다. 미스 적재는 `meogo-application`이 스캔 유스케이스에서 `research`에 등록한다.
- `member`/`assessment`: 직접 관련 없다.
- `meogo-infra`: LLM 병렬 호출의 기술 구현(core port 어댑터). 호출 자체는 `meogo-application`이 port로 트리거한다.
