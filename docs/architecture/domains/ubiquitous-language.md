...
| Value Object (VO) | 값 객체 | 식별자 없이 값 자체로 의미를 갖는 불변 객체(예: `AvoidanceInput`). |
...
| `member` | 사용자가 누구이고 어떤 식이 제한·선호를 갖는지 | `Member`, `DietaryProfile` | 회원 기본 정보·인증 상태, 프로필(닉네임·국적), 식이 제한(알러지·종교·비건), 매운맛 허용도, 관심 음식, 랭킹. **음식 재료나 위험도 계산은 담지 않는다.** |
...
| `avoidance` | 특정 사용자에게 특정 음식이 안전한지 판정 | `AvoidanceResult` 또는 도메인 결과 객체 (정책 도메인) | 위험도(`RiskLevel`), 위험 사유, 재료별 위험, 사장님 질문에 필요한 값. 입력은 전용 VO로만 받고 결과는 도메인 결과 객체로 반환한다. `AvoidanceResult`를 영속 생명주기로 관리할 때만 Aggregate Root로 본다. **식이 제한·음식 데이터를 저장하지 않는다.** |
...
classDiagram
    ...
    AvoidanceResult "1" *-- "*" AvoidanceReason
    AvoidanceResult "1" *-- "*" IngredientRisk
    AvoidanceResult "1" *-- "*" OwnerQuestion
    AvoidanceResult --> RiskLevel
    AvoidancePolicy ..> AvoidanceInput : 입력
    AvoidancePolicy ..> AvoidanceResult : 출력
    AvoidanceInput "1" *-- "*" DietaryCondition
    AvoidanceInput "1" *-- "*" IngredientAssessmentSource
    ScanAssessmentSnapshot ..> AvoidanceResult : 결과 스냅샷
```

> 다이어그램이 보여주는 핵심 규칙: `avoidance`는 `food`/`member`의 클래스에 **직접 화살표가 없다.** `:application`이 `DietaryProfile`과 `FoodIngredient` 등 원천 데이터를 `AvoidanceInput` 안의 전용 값(`DietaryCondition`, `IngredientAssessmentSource`)으로 변환한다. `scan`은 음식 정보를 복제하지 않고 `foodId`와 결과 **스냅샷**만 들고 있다.
...
| `MemberProfile` | 회원 프로필 | 닉네임·국적·프로필 이미지. 표시 언어는 프로필이 아니라 요청 `lang` 이 정한다(KB-229). |
...
| `AvoidanceResult` | 판정 결과 | 음식 하나에 대한 사용자별 도메인 결과 객체. 위험도·위험 사유·재료별 위험·사장님 질문 후보·판정 시각. 별도 저장소에서 현재 판정 결과를 생명주기 관리할 때만 **Aggregate Root**로 본다. 스캔 당시 보존 값은 `scan`의 `ScanAssessmentSnapshot`이 소유한다. |
| `AvoidanceInput` | 판정 입력 | **Value Object.** Application이 만들어 넘기는 입력. 식이 제한 조건 + 음식 재료 목록 + 포함 스코어 + 알러지/종교/비건 매핑 + 원문 메뉴명. **다른 BC의 영속 모델을 직접 받지 않음.** |
| `DietaryCondition` | 판정용 사용자 제한 조건 | `AvoidanceInput` 내부 전용 값. `member`의 `DietaryProfile`을 직접 참조하지 않고 Application이 필요한 제한 유형·코드·심각도만 변환한다. |
| `IngredientAssessmentSource` | 판정용 재료 입력 | `AvoidanceInput` 내부 전용 값. `food`의 `FoodIngredient`를 직접 참조하지 않고 Application이 재료 ID·재료명·포함 가능성 스코어·제한 매핑만 변환한다. |
| `AvoidanceReason` | 위험 사유 | 원인 재료·원인 제한 조건·위험도·포함 가능성 스코어. `CAUTION`은 "매장별로 다를 수 있음" 맥락 포함. |
...
| `AvoidancePolicy` | 위험도 계산 정책 | 음식/재료 판정 대상이 아니면 `UNKNOWN`. 판정 가능한 음식에서는 여러 사유 중 **가장 높은 위험도 채택.** `DANGER` 하나라도 있으면 `DANGER`, 없고 `CAUTION` 있으면 `CAUTION`. |
...
| 음식 콘텐츠 언어 | 한국어 원문 + 9개 언어 | 음식명·설명·재료명·주의 성분을 `ko` 원문 + 9개 언어(`zh-Hans`/`en`/`ja`/`zh-Hant`/`vi`/`id`/`th`/`ru`/`es`)로 사전 번역 저장(ADR-0003). 요청 `lang` 이 어느 번역본을 내려줄지 결정(KB-229). |
