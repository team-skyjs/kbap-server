...
| `deploy-batch-dev.yml` | develop push(배치 경로 변경 시) · 수동 | dev 배치 이미지·태스크정의 갱신 |
| `run-batch.yml` | 수동(환경 선택) | 선택 환경에서 태스크 1회 기동(fire-and-forget) |
| EventBridge Scheduler | 매시 17분 (인프라 소유) | prod 시간별 자동 실행 |

- 빌드와 실행이 분리되어 있다: 이미지는 코드가 바뀔 때만 굽고, 실행은 등록된 최신
  태스크정의를 기동만 한다. GitHub Actions 러너가 실행을 관찰하지 않는다(비용 0 유지).
...
## 최초 1회: 태스크정의 등록 (인프라 소유, 환경별)

파이프라인은 기존 리비전을 복제해 이미지만 바꾸므로, family 최초 등록은 수동이다.
아래 템플릿의 `<...>` 를 채워 환경별로 등록한다(dev 는 family·로그그룹·SSM 경로만 교체):
...
OIDC 롤(`AWS_ROLE_ARN`)에는 기존 권한에 더해 `ecs:RunTask`·`ecs:DescribeTaskDefinition`·
