...
class OpenAiMenuBoardVisionExtractor(
    ...
    override fun extract(
        ...
        val userMessage = UserMessage.builder().text(userPromptWith(ocrItems, mode)).media(media).build()

        val response = chatModel.call(Prompt(listOf(SystemMessage(systemPromptOf(mode)), userMessage)))
    ...
    private fun systemPromptOf(mode: MenuBoardReadingMode): String =
        when (mode) {
            MenuBoardReadingMode.PHOTO_ONLY -> PHOTO_ONLY_SYSTEM_PROMPT
            MenuBoardReadingMode.OCR_ASSISTED -> OCR_ASSISTED_SYSTEM_PROMPT
        }

    private fun userPromptWith(ocrItems: List<OcrItem>, mode: MenuBoardReadingMode): String {
        val ocrLines = ocrItems.joinToString("\n") { "${it.idx}: ${it.rawMenuName}" }
        val header = when (mode) {
            MenuBoardReadingMode.PHOTO_ONLY -> PHOTO_ONLY_USER_HEADER
            MenuBoardReadingMode.OCR_ASSISTED -> OCR_ASSISTED_USER_HEADER
        }
        return "$header\n$ocrLines"
    }
    ...
    companion object {
        ...
        private val OCR_ASSISTED_SYSTEM_PROMPT = """
        ...
        private val PHOTO_ONLY_SYSTEM_PROMPT = """
