...
private fun descriptionJson(description: String, translations: Map<String, String> = NINE_TRANSLATIONS): String {
    val t = translations.entries.joinToString(", ") { "\"${it.key}\": \"${it.value}\"" }
    return """{"description": "$description", "translations": {$t}}"""
}

class SpringAiFoodDescriptionClientTest : BehaviorSpec({
    given("정상 설명 응답을 주는 caller") {
        val caller = DescStubCaller(descriptionJson("불고기는 달콤 짭짤한 한국식 소고기 요리다."))
        val client = SpringAiFoodDescriptionClient(caller)

        `when`("설명 생성을 호출하면") {
            val result = client.call("불고기")
            then("설명·9개 번역을 담은 콘텐츠를 반환하고 프롬프트에 맵기 항목이 없다") {
                result.description shouldBe "불고기는 달콤 짭짤한 한국식 소고기 요리다."
                result.translations.texts.size shouldBe 9
                caller.lastPrompt!! shouldNotContain "spiciness"
            }
        }
    }

    given("256자 설명(255자 초과)을 주는 caller") {
        val client = SpringAiFoodDescriptionClient(DescStubCaller(descriptionJson("가".repeat(256))))
    ...
    given("번역 언어를 누락한 응답을 주는 caller") {
        val client = SpringAiFoodDescriptionClient(
            DescStubCaller(descriptionJson("설명", NINE_TRANSLATIONS - "ja")),
        )

        `when`("설명 생성을 호출하면") {
            then("9개 전수 위반으로 예외를 전파한다") {
                shouldThrow<IllegalArgumentException> { client.call("불고기") }
            }
        }
    }
    ...
    given("플레이스홀더 설명을 주는 caller") {
        val client = SpringAiFoodDescriptionClient(DescStubCaller(descriptionJson("설명 준비 중")))
