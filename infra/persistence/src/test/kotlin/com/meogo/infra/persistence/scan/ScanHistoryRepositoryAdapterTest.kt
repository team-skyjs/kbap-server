...
class ScanHistoryRepositoryAdapterTest : BehaviorSpec() {
    override fun extensions() = listOf(SpringExtension)

    @Autowired
    private lateinit var adapter: ScanHistoryRepositoryAdapter
        ...
        given("최근 스캔 음식 조회") {
            `when`("같은 음식을 여러 번 스캔했으면") {
                then("가장 최근 1건만 남기고 최신순으로 반환한다") {
                    ...
                    adapter.findRecentReadyFoodIds(memberId = 11L, limit = 10) shouldContainExactly listOf(1L, 2L)
                }
            }
            ...
            `when`("완성(READY)되지 않은 음식 이력이 섞여 있으면") {
                then("READY 음식만 반환한다") {
                    ...
                    adapter.findRecentReadyFoodIds(memberId = 11L, limit = 10) shouldContainExactly listOf(1L)
                }
            }
            ...
            `when`("이력이 limit 보다 많으면") {
                then("최신순 limit 개만 반환한다") {
                    ...
                    val result = adapter.findRecentReadyFoodIds(memberId = 11L, limit = 10)
            ...
            `when`("이력이 없으면") {
                then("빈 목록을 반환한다") {
                    adapter.findRecentReadyFoodIds(memberId = 11L, limit = 10) shouldBe emptyList()
                }
            }
        }
    }
