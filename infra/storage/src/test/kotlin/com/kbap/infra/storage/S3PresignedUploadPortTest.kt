...
class S3PresignedUploadPortTest : BehaviorSpec({
    val presigner = S3Presigner.builder()
        ...
        .build()
    val port = S3PresignedUploadPort(presigner, bucket = "kbap-test-bucket", publicBaseUrl = "https://cdn.test")
    ...
    given("S3 presigned 업로드 발급 — 로컬 SigV4 서명(네트워크 없음)") {
        ...
        `when`("공개 베이스 URL 이 슬래시로 끝나도") {
            then("공개 URL 에 슬래시가 중복되지 않는다") {
                val trailingPort = S3PresignedUploadPort(presigner, bucket = "kbap-test-bucket", publicBaseUrl = "https://cdn.test/")
